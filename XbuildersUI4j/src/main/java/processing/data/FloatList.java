...
public class FloatList implements Iterable<Float> {
  ...
  public FloatList(Iterable<Object> iterator) {
    ...
    for (Object o : iterator) {
      if (o == null) {
        ...
        append(UIFrame.parseFloat(o.toString().trim()));
      }
    }
    crop();
  }
    ...
    for (Object o : items) {
      ...
      if (o != null) {
        if (o instanceof Number) {
          ...
          value = UIFrame.parseFloat(o.toString().trim(), missingValue);
        }
      }
      data[index++] = value;
    }
  ...
  private void crop() {
    if (count != data.length) {
      data = UIFrame.subset(data, 0, count);
    }
  }
  ...
  public void set(int index, float what) {
    if (index >= count) {
      data = UIFrame.expand(data, index+1);
...
//    }
   ...
   */
  @SuppressWarnings("unused")
  public void shuffle(UIFrame sketch) {
    int num = count;
    while (num > 1) {
...
//   */
//  public float random(UIFrame sketch) {
//    if (count == 0) {
//      throw new ArrayIndexOutOfBoundsException("No entries in this FloatList");
//    }
