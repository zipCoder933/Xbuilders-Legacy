...
public class Texture implements PConstants {
  ...
  protected void updateTexelsImpl(int x, int y, int w, int h) {
    ...
    if (!modified) {
      mx1 = UIFrame.max(0, x);
      mx2 = UIFrame.min(width - 1, x2);
      my1 = UIFrame.max(0, y);
      my2 = UIFrame.min(height - 1, y2);
      modified = true;

    } else {
      if (x < mx1) mx1 = UIFrame.max(0, x);
      if (x > mx2) mx2 = UIFrame.min(width - 1, x);
      if (y < my1) my1 = UIFrame.max(0, y);
      if (y > my2) my2 = y;

      if (x2 < mx1) mx1 = UIFrame.max(0, x2);
      if (x2 > mx2) mx2 = UIFrame.min(width - 1, x2);
      if (y2 < my1) my1 = UIFrame.max(0, y2);
      if (y2 > my2) my2 = UIFrame.min(height - 1, y2);
    }
  }
