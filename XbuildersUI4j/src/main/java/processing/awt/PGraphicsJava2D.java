...
//        parent.doLayout();
        ...
        if (canvas.getWidth() != width || canvas.getHeight() != height) {
          UIFrame.debug("PGraphicsJava2D comp size being set to " + width + "x" + height);
          canvas.setSize(width, height);
        } else {
          UIFrame.debug("PGraphicsJava2D comp size already " + width + "x" + height);
        }
  ...
  public void beginDraw() {
    ...
    if (primarySurface && !useOffscreen) {
      ...
      if (false) {
        ...
        if (image == null) {
          image = gc.createCompatibleImage(width, height);
          UIFrame.debug("created new image, type is " + image);
          g2 = (Graphics2D) image.getGraphics();
          reapplySettings = true;
        }
      }
    ...
    if (useCanvas && primarySurface) {
      if (parent.frameCount == 0) {
        ...
        UIFrame.debug("PGraphicsJava2D.beginDraw() strategy is " + strategy);
        BufferCapabilities caps = strategy.getCapabilities();
        caps = strategy.getCapabilities();
        UIFrame.debug("PGraphicsJava2D.beginDraw() caps are " +
      ...
      if (bimage == null ||
        ...
        UIFrame.debug("PGraphicsJava2D creating new image");
...
//    if (strategy == null) return;
    do {
      UIFrame.debug("PGraphicsJava2D.redraw() top of outer do { } block");
      do {
        UIFrame.debug("PGraphicsJava2D.redraw() top of inner do { } block");
        UIFrame.debug("strategy is " + strategy);
...
//    }
      ...
      UIFrame.debug("PGraphicsJava2D.redraw() showing strategy");
      strategy.show();

    } while (strategy.contentsLost());
    UIFrame.debug("PGraphicsJava2D.redraw() out of do { } block");
  }
  */
...
//          op.filter(image, image);
  ...
  public PShape loadShape(String filename, String options) {
    String extension = UIFrame.getExtension(filename);
