...
public class PShapeJava2D extends PShapeSVG {
  ...
  @Override
  public void setParent(PShapeSVG parent) {
  ...
  @Override
  public PShapeSVG createShape(PShapeSVG parent, XML properties, boolean parseKids) {
  ...
  */
  ...
  static class LinearGradientPaint implements Paint {
    ...
    public class LinearGradientContext implements PaintContext {
      ...
      public Raster getRaster(int x, int y, int w, int h) {
        ...
        int span = (int) UIFrame.dist(tx1, ty1, tx2, ty2) * ACCURACY;
        if (span <= 0) {
          ...
          for (int i = 1; i < count; i++) {
            ...
            for (int j = prev; j <= last; j++) {
              float btwn = UIFrame.norm(j, prev, last);
              interp[j][0] = (int) UIFrame.lerp((c0 >> 16) & 0xff, (c1 >> 16) & 0xff, btwn);
              interp[j][1] = (int) UIFrame.lerp((c0 >> 8) & 0xff, (c1 >> 8) & 0xff, btwn);
              interp[j][2] = (int) UIFrame.lerp(c0 & 0xff, c1 & 0xff, btwn);
              interp[j][3] = (int) (UIFrame.lerp((c0 >> 24) & 0xff, (c1 >> 24) & 0xff, btwn) * opacity);
          ...
          for (int j = 0; j < h; j++) {
            for (int i = 0; i < w; i++) {
              ...
              //int which = UIFrame.min((int) (distance * ACCURACY), interp.length-1);
              float px = (x + i) - tx1;
              float py = (y + j) - ty1;
  ...
  static class RadialGradientPaint implements Paint {
    ...
    public class RadialGradientContext implements PaintContext {
      ...
      public Raster getRaster(int x, int y, int w, int h) {
        ...
        for (int j = 0; j < h; j++) {
          for (int i = 0; i < w; i++) {
            float distance = UIFrame.dist(cx, cy, x + i, y + j);
            int which = UIFrame.min((int) (distance * ACCURACY), interp.length-1);
  ...
  public Paint calcGradientPaint(Gradient gradient) {
