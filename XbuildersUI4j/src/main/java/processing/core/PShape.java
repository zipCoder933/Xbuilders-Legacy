 ...
 * UIFrame exist as placeholders for more exciting things to come. If you'd like
 ...
 * @see UIFrame#shapeMode(int)
 */
public class PShape implements PConstants {

    public String name;
    public Map<String, PShape> nameTable;
...
//  static public final int GROUP = 0;
     ...
     */
    public boolean openShape = false;

    public boolean openContour = false;
    ...
    public int sphereDetailU, sphereDetailV;
    public int rectMode;
    ...
    public PShape parent;
    public int childCount;
    public PShape[] children;

    /**
     * Array of VERTEX, BEZIER_VERTEX, and CURVE_VERTEX calls.
     */
    public int vertexCodeCount;
    ...
    public boolean perVertexStyles = false;
...
//      shapeCreated = true;
    ...
    /**
     * Returns a boolean value <b>true</b> if the image is set to be visible,
     * <b>false</b> if not. This value can be modified with the
     * <b>setVisible()</b> method.<br />
     * <br />
     * The default visibility of a shape is usually controlled by whatever
     * program created the SVG file. For instance, this parameter is controlled
     * by showing or hiding the shape in the layers palette in Adobe
     * Illustrator.
     *
     * @webref pshape:method
     * @usage web_application
     * @webBrief Returns a boolean value <b>true</b> if the image is set to be
     * visible,
     * <b>false</b> if not
     ...
     * @webref pshape:method
     * @usage web_application
     * @webBrief Enables the shape's style data and ignores the Processing
     * styles
...
//  }
     ...
     * possible to define shapes with other shapes cut out of them. For example,
     * the inside of a letter 'O'. These two functions are always used together,
     * you'll never use one without the other. Between them, define the geometry
     * you want to create. As you'll see when you run the example above, the
     ...
     * opposite directions. This means that if the points of the geometry for
     * the exterior shape are described in a clockwise order, the points on the
     * interior shape are defined in a counterclockwise order.
     *
     * @webref shape:vertex
     ...
     * in opposite directions. This means that if the points of the geometry for
     * the exterior shape are described in a clockwise order, the points on the
     * interior shape are defined in a counterclockwise order.
     *
     * @webref shape:vertex
  ...
  int shapeModeSaved;
    ...
    public void styles(PGraphics g) {
        ...
        if (fill) {
            //System.out.println("filling " + UIFrame.hex(fillColor));
            g.fill(fillColor);
        } else {
            g.noFill();
        }
...
//    }
    ...
    // Shape copy
    // TODO unapproved
    static public PShape createShape(UIFrame parent, PShape src) {
        PShape dest = null;
        if (src.family == GROUP) {
    ...
    // TODO unapproved
    static public void copyGroup(UIFrame parent, PShape src, PShape dest) {
        copyMatrix(src, dest);
        copyStyles(src, dest);
        copyImage(src, dest);
...
//        dest.shininess(vert[PGraphics.SHINE]);
                if (0 < UIFrame.dist(vert[PGraphics.NX],
                        vert[PGraphics.NY],
...
//      drawPath(g);
    ...
    public void drawGroup(PGraphics g) {
        for (int i = 0; i < childCount; i++) {
            children[i].draw(g);
        }
    }

    public void drawPrimitive(PGraphics g) {
        if (kind == POINT) {
  ...
  public void drawPath(PGraphics g) {
     ...
     */
    static public PImage parseBase64Image(String imagePath) {
...
//    }
     ...
     * example, the three shapes <b>path</b>, <b>rectangle</b>, and
     * <b>circle</b> are added to a parent PShape variable named
    ...
    public void addChild(PShape who) {
        ...
        if (childCount == children.length) {
            children = (PShape[]) UIFrame.expand(children);
        }
    ...
    /**
     * Returns a PShape holding the tessellated geometry of this shape, composed
    ...
    public float[] getParams(float[] target) {
        ...
        UIFrame.arrayCopy(params, target);
        return target;
    }
    ...
    public void setParams(float[] source) {
        ...
        UIFrame.arrayCopy(source, params);
    }
    ...
    public void setPath(int vcount, float[][] verts, int ccount, int[] codes) {
        ...
        for (int i = 0; i < vertexCount; i++) {
            UIFrame.arrayCopy(verts[i], vertices[i]);
        }

        vertexCodeCount = ccount;
        if (0 < vertexCodeCount) {
            vertexCodes = new int[vertexCodeCount];
            UIFrame.arrayCopy(codes, vertexCodes, vertexCodeCount);
        }
    }

    /**
     * The <b>getVertexCount()</b> method returns the number of vertices that
     * make up a <b>PShape</b>. In the above example, the value 4 is returned by
     * the
    ...
    /**
     * The <b>getVertex()</b> method returns a <b>PVector</b> with the
     * coordinates of the vertex point located at the position defined by the
     * <b>index</b>
     ...
     * located at the position defined by the <b>index</b> parameter. This
     * method works when shapes are created as shown in the example above, but
     * won't work properly when a shape is defined explicitly (e.g.
    ...
    public int[] getVertexCodes() {
        ...
        if (vertexCodes.length != vertexCodeCount) {
            vertexCodes = UIFrame.subset(vertexCodes, 0, vertexCodeCount);
        }
        return vertexCodes;
    }
     ...
     * with PATH shapes or GROUP shapes that contain other GROUPs or PATHs. This
     * method is not imperfect and doesn't account for all cases (not all
     * complex shapes: concave shapes or holes may have issues).
     */
