 ...
 * registerXxx() methods in UIFrame can help ensure that your code doesn't do
 * something naughty.
 * <p/>
 * As of Processing 3.0, we have removed Applet as the base class for UIFrame.
 * This means that we can remove lots of legacy code, however one downside is
 * that it's no longer possible (without extra code) to embed a UIFrame into
...
public abstract class UIFrame implements PConstants {
    ...
    /**
     * Sets the window location to a specific point on the screen
     *
     * @param x the x coordinate of the window
     * @param y the y coordinate of the window
     */
    public final void setWindowLocation(int x, int y) {
        surface.setLocation(x, y);
    }

    /**
     * Determines if the window will always stay on top of other windows
     *
     * @param alwaysOnTop true if the window should always stay on top, false
     * otherwise
     */
    ...
    public final void startWindow() {
        runSketch(new String[]{this.getClass().getSimpleName()}, this);
    }

//public class UIFrame extends PSketch {  // possible in the next alpha
    /**
    ...
    /**
     * The PGraphics renderer associated with this UIFrame
     */
    public PGraphics g;
     ...
     * @see UIFrame#displayHeight
     * @see UIFrame#size(int, int)
     */
    ...
    /**
     * Path to sketch folder. Previously undocumented, and made private in 3.0
     * alpha 5 so that people use the sketchPath() method which will initialize
     * it properly. Call sketchPath() once to set it.
     */
...
//  static final boolean DEBUG = true;
     ...
     * 100 x 100 pixels, there will be 10,000 values and if the window is 200 x
     * 300 pixels, there will be 60,000 values. When the pixel density is set to
     * higher than 1 with the <b>pixelDensity()</b> function, these values will
     * change. See the reference for <b>pixelWidth</b> or <b>pixelHeight</b>
     * for more information.
     * <br /><br />
     * Before accessing this array, the data must be loaded with the
     * <b>loadPixels()</b>
     * function. Failure to do so may result in a NullPointerException.
     * Subsequent changes to the display window will not be reflected in
     * <b>pixels</b> until
     * <b>loadPixels()</b> is called again. After <b>pixels</b> has been
     * modified, the <b>updatePixels()</b> function must be run to update the
     * content of the display window.
     *
     * @webref image:pixels
     * @webBrief Array containing the values for all the pixels in the display
     * window
     * @see UIFrame#loadPixels()
     * @see UIFrame#updatePixels()
     * @see UIFrame#get(int, int, int, int)
     * @see UIFrame#set(int, int, int)
     * @see PImage
     * @see UIFrame#pixelDensity(int)
     * @see UIFrame#pixelWidth
     * @see UIFrame#pixelHeight
     */
    public int[] pixels;
     ...
     * is over the current window. The default value of <b>mouseX</b> is
     * <b>0</b>, so <b>0</b> will be returned until the mouse moves in front of
     * the sketch window. (This typically happens when a sketch is first run.)
     * Once the mouse moves away from the window, <b>mouseX</b> will continue to
     * report its most recent position.
     *
     * @webref input:mouse
     ...
     * @see UIFrame#pmouseX
     * @see UIFrame#pmouseY
     * @see UIFrame#mousePressed
     * @see UIFrame#mousePressed()
     * @see UIFrame#mouseReleased()
     * @see UIFrame#mouseClicked()
     * @see UIFrame#mouseMoved()
     * @see UIFrame#mouseDragged()
     * @see UIFrame#mouseButton
     * @see UIFrame#mouseWheel(MouseEvent)
     */
    public int mouseX;
     ...
     * <br /><br />
     * Note that Processing can only track the mouse position when the pointer
     * is over the current window. The default value of <b>mouseY</b> is
     * <b>0</b>, so <b>0</b> will be returned until the mouse moves in front of
     * the sketch window. (This typically happens when a sketch is first run.)
     * Once the mouse moves away from the window, <b>mouseY</b> will continue to
     * report its most recent position.
     *
     * @webref input:mouse
     ...
     * <b>pmouseX</b> and <b>pmouseY</b> update only once per frame (once per
     * trip through the <b>draw()</b> loop). But inside mouse events, they
     * update each time the event is called. If these values weren't updated
     * immediately during events, then the mouse position would be read only
     * once per frame, resulting in slight delays and choppy interaction. If the
     * mouse variables were always updated multiple times per frame, then
     * something like <b>line(pmouseX, pmouseY, mouseX, mouseY)</b> inside
     * <b>draw()</b> would have lots of gaps, because
     ...
     * pressed. The <b>mouseButton</b> variable (see the related reference
     ...
     * and will accept input from mouse or keyboard. This variable is
     * <b>true</b> if it is focused and <b>false</b> if not.
     *
     * @webref environment
     ...
     *
     * @webref environment
     * @webBrief The system variable that contains the approximate frame rate of
     * the software as it executes
     * @see UIFrame#frameRate(float)
     * @see UIFrame#frameCount
     */
    ...
    /**
     * The system variable <b>frameCount</b> contains the number o frames
     * displayed since the program started. Inside <b>setup()</b>
    ...
    /**
     * Position of the upper left-hand corner of the editor window that launched
     * this sketch.
     */
     ...
     * <p>
     * This is used by the editor to when saving the previous sketch location,
...
//  static public final String ARGS_SPAN_DISPLAYS = "--span";
    ...
    /**
     * When run externally to a PDE Editor, this is sent by the sketch whenever
     * the window is moved.
     * <p>
    ...
    private PImage defaultRendererIcon = null;

    public void setIcon(File file) {
        if (insideSettings("setIcon", file)) {
            if (file.getName().toLowerCase().endsWith(".png")) {
                if (renderer.equals(PConstants.JAVA2D)) {
                    defaultRendererIcon = loadImage(file.getAbsolutePath());
                } else {
                    PJOGL.setIcon(file.getAbsolutePath());
                }
            } else {
                throw new IllegalArgumentException(
                        String.format("Icon image \"%s\" must be a .PNG file", file.getName()));
            }
        }
    }
    ...
    /**
     * The <b>settings()</b> function is new with Processing 3.0. It's not
     * needed in most sketches. It's only useful when it's absolutely necessary
     * to define the parameters to <b>size()</b>
     * with a variable. Alternately, the <b>settings()</b> function is necessary
     * when using Processing code outside the Processing Development Environment
     * (PDE). For example, when using the Eclipse code editor, it's necessary to
     * use
     * <b>settings()</b> to define the <b>size()</b> and
     * <b>smooth()</b> values for a sketch.</b>.
     * <br /> <br />
     * The <b>settings()</b> method runs before the sketch has been set up, so
     * other Processing functions cannot be used at that point. For instance, do
     * not use loadImage() inside settings(). The settings() method runs
     * "passively" to set a few variables, compared to the <b>setup()</b>
    ...
    /**
     * This function makes it possible to render using all the pixels on high
     * resolutions screens like Apple Retina and Windows HiDPI. This function
     * can only be run once within a program, and must be called right after
     * <b>size()</b> in a program without a
     * <b>setup()</b> function, or within <b>setup()</b> if present.
     * <p/>
     * <b>pixelDensity()</b> should only be used with hardcoded numbers (in
     * almost all cases this number will be 2) or in combination with
     * <b>displayDensity()</b> as in the third example above.
     * <p/>
     * When the pixel density is set to more than 1, it changes the pixel
     * operations including the way <b>get()</b>, <b>set()</b>,
     * <b>blend()</b>, <b>copy()</b>, and <b>updatePixels()</b>
     * all work. See the reference for <b>pixelWidth</b> and pixelHeight for
     * more information.
     * <p/>
     * To use variables as the arguments to <b>pixelDensity()</b>
     * function, place the <b>pixelDensity()</b> function within the
     * <b>settings()</b> function. There is more information about this on the
     * <b>settings()</b> reference page.
     *
     * @webref environment
     ...
     * default, so <b>smooth()</b> only needs to be used when a program needs to
     * set the smoothing in a different way. The level parameter increases the
     * amount of smoothness. This is the level of over sampling applied to the
     * graphics buffer.
     * <p/>
     * With the P2D and P3D renderers, <b>smooth(2)</b> is the default, this is
     * called "2x anti-aliasing." The code
     * <b>smooth(4)</b> is used for 4x anti-aliasing and <b>smooth(8)</b>
     * is specified for "8x anti-aliasing." The maximum anti-aliasing level is
     * determined by the hardware of the machine that is running the software,
     * so <b>smooth(4)</b>
     * and <b>smooth(8)</b> will not work with every computer.
     * <p/>
     * The default renderer uses <b>smooth(3)</b> by default. This is bicubic
     * smoothing. The other option for the default renderer is <b>smooth(2)</b>,
     * which is bilinear smoothing.
     * <p/>
     * With Processing 3.0, <b>smooth()</b> is handled differently than in
     * earlier releases. In 2.x and earlier, it was possible to use
     * <b>smooth()</b> and <b>noSmooth()</b> to turn on and off antialiasing
     * within a sketch. Now, because of how the software has changed,
     * <b>smooth()</b> can only be set once within a sketch. It can be used
     * either at the top of a sketch without a <b>setup()</b>, or after the
     * <b>size()</b>
     * function when used in a sketch with <b>setup()</b>. The
     * <b>noSmooth()</b> function also follows the same rules.
     * <p/>
     * When <b>smooth()</b> is used with a PGraphics object, it should be run
     * right after the object is created with
     * <b>createGraphics()</b>, as shown in the Reference in the third example.
     *
     * @webref environment
    ...
    /**
     * Draws all geometry and fonts with jagged (aliased) edges and images with
     * hard edges between the pixels when enlarged rather than interpolating
     * pixels. Note that <b>smooth()</b> is active by default, so it is
     * necessary to call <b>noSmooth()</b> to disable smoothing of geometry,
     * fonts, and images. Since the release of Processing 3.0, the
     * <b>noSmooth()</b> function can only be run once for each sketch, either
     * at the top of a sketch without a <b>setup()</b>, or after the
     * <b>size()</b> function when used in a sketch with
     * <b>setup()</b>. See the examples above for both scenarios.
     *
     * @webref environment
     * @webBrief Draws all geometry and fonts with jagged (aliased) edges and
     * images with hard edges between the pixels when enlarged rather than
    ...
    /**
     * Called by the application context to start running (or resume) the
     * sketch.
     */
    public void start() {
        resume();
...
//        surface.stopThread();
    ...
    class RegisteredMethods {
        ...
        /**
         * List of the objects for which the method is registered. This is an
         * ordered collection because the order of calls likely matters, or at a
         * minimum, needs to be stable.
         */
         ...
         * called multiple times if object is registered multiple times).
         */
     ...
     * draw)
     * <ul>
     * In addition, the new (for 2.0) <tt>processing.event</tt> classes are
     * passed to the following event types:
     * <ul>
     * <li>mouseEvent
     * <li>keyEvent
     * <li>touchEvent
    ...
    /**
     *
     * The <b>setup()</b> function is run once, when the program starts. It's
     * used to define initial environment properties such as screen size and to
     * load media such as images and fonts as the program starts. There can only
     * be one
     * <b>setup()</b> function for each program, and it shouldn't be called
     * again after its initial execution.<br />
     * <br />
     * If the sketch is a different dimension than the default, the
     * <b>size()</b>
     ...
     *
     * @webref structure
     * @webBrief The <b>setup()</b> function is called once when the program
     * starts
    ...
    /**
     *
     * Called directly after <b>setup()</b>, the <b>draw()</b> function
     * continuously executes the lines of code contained inside its block until
     * the program is stopped or <b>noLoop()</b> is called. <b>draw()</b> is
     * called automatically and should never be called explicitly. All
     * Processing programs update the screen at the end of draw(), never
     * earlier.<br />
     * <br />
     ...
     * continuously.<br />
     * <br />
     * The number of times <b>draw()</b> executes in each second may be
     * controlled with the <b>frameRate()</b> function.<br />
     * <br />
     * It is common to call <b>background()</b> near the beginning of the
     * <b>draw()</b> loop to clear the contents of the window, as shown in the
     * first example above. Since pixels drawn to the window are cumulative,
     * omitting
     ...
     * such as <b>mousePressed()</b>. Sometimes, you might have an empty call to
     * <b>draw()</b> in your program, as shown in the second example above.
     *
     * @webref structure
     * @webBrief Called directly after <b>setup()</b> and continuously executes
  ...
  protected void resizeRenderer(int newWidth, int newHeight) {
     ...
     * <b>setup()</b>. The <b>size()</b> and <b>fullScreen()</b> functions
     * cannot both be used in the same program, just choose one.<br />
     * <br />
     * When <b>fullScreen()</b> is used without a parameter, it draws the sketch
     * to the screen currently selected inside the Preferences window. When it
     * is used with a single parameter, this number defines the screen to
     * display to program on (e.g. 1, 2, 3...). When used with two parameters,
     * the first defines the renderer to use (e.g. P2D) and the second defines
     * the screen. The <b>SPAN</b> parameter can be used in place of a screen
     * number to draw the sketch as a full-screen window across all the attached
     * displays if there are more than one.<br />
     * <br />
     ...
     * <b>setup()</b>, and the <b>setup()</b> function must appear in the code
     * tab with the same name as your sketch folder.<br />
     * <br />
     ...
     * 480)</b> will assign 640 to the <b>width</b> variable and 480 to the
     * height
     ...
     * <br />
     * To run a sketch that fills the screen, use the <b>fullScreen()</b>
     * function, rather than using <b>size(displayWidth,
     * displayHeight)</b>.<br />
     * <br />
     ...
     * high-resolution displays) but generally has higher quality than the other
     ...
     * faster for some applications, but has some compatibility quirks. Use
     * “Manage Libraries” to download and install the JavaFX library.<br />
     * <br />
     ...
     * Like PDF, you must first use Import Library &rarr; SVG Export to make use
     * the SVG library.<br />
     * <br />
     ...
     * function (instead of <b>setup()</b>). There is more information about
     * this on the <b>settings()</b> reference page.<br />
     * <br />
     ...
     * screen of 800 x 600 could support <b>size(1600, 300)</b>, since that is
     * the same number of pixels. This varies widely, so you'll have to try
     ...
     * enforce the minimum size so that sketches will run identically on
     ...
     * specifies the renderer. It can be defined as P2D, P3D, PDF, or SVG. If
     * the third parameter isn't used, the default renderer is set. The PDF and
     * SVG renderers require the filename parameter.<br />
     * <br />
     ...
     * it's only possible to use P2D or P3D with <b>createGraphics()</b> when
     * one of them is defined in <b>size()</b>. Unlike Processing 1.0, P2D and
     * P3D use OpenGL for drawing, and when using an OpenGL renderer it's
     * necessary for the main drawing surface to be OpenGL-based. If P2D or P3D
     * are used as the renderer in <b>size()</b>, then any of the options can be
     * used with
     * <b>createGraphics()</b>. If the default renderer is used in
     * <b>size()</b>, then only the default, PDF, or SVG can be used with
     * <b>createGraphics()</b>.<br />
     * <br />
     * It's important to run all drawing functions between the
     * <b>beginDraw()</b>
     * and <b>endDraw()</b>. As the exception to this rule, <b>smooth()</b>
     * should be run on the PGraphics object before <b>beginDraw()</b>. See the
     * reference for <b>smooth()</b> for more detail.<br />
     * <br />
     * The <b>createGraphics()</b> function should almost never be used inside
     * <b>draw()</b> because of the memory and time needed to set up the
     * graphics. One-time or occasional use during <b>draw()</b> might be
     * acceptable, but code that calls <b>createGraphics()</b> at 60 frames per
     * second might run out of memory or freeze your sketch.<br />
     * <br />
     ...
     * graphics context back to its parent UIFrame.
     ...
     * ({@link UIFrame#savePath} will be called)
     */
...
//      }
     ...
     * to the parent UIFrame is included, which makes save() work without
    ...
    public void handleDraw() {
        ...
        if (frameCount == 0) {
            if (defaultRendererIcon != null) {
                surface.setIcon(defaultRendererIcon);
            }
            setup();
...
//  }
     ...
     * when an event registered by <b>mousePressed()</b> or <b>keyPressed()</b>
     * occurs.
     * <br/>
     * <br/>
     * In structuring a program, it only makes sense to call redraw() within
     * events such as <b>mousePressed()</b>. This is because <b>redraw()</b>
     * does not run
     * <b>draw()</b> immediately (it only sets a flag that indicates an update
     * is needed). <br/>
     * <br/>
...
//      }
    ...
    /**
     *
     * By default, Processing loops through <b>draw()</b> continuously,
     * executing the code within it. However, the <b>draw()</b> loop may be
     * stopped by calling
    ...
    protected void handleMouseEvent(MouseEvent event) {
        ...
        // Handled inside UIFrame so that the same logic need not be redone
        ...
        if (UIFrame.platform == PConstants.MACOS
    ...
    private HashMap<KeyCode, Boolean> keyCodeMap;

    public boolean keyIsPressed(KeyCode keyCode2) {
        Boolean val = keyCodeMap.get(keyCode2);
        return val != null && val == true;
    }

    public boolean keysArePressed(KeyCode... keyCode2) {
        for (int i = 0; i < keyCode2.length; i++) {
            Boolean val = keyCodeMap.get(keyCode2[i]);
    ...
    protected void handleKeyEvent(KeyEvent event) {
        ...
        switch (event.getAction()) {
            case KeyEvent.PRESS -> {
                keyCodeMap.put(event.getKeyCode(), true);
                keyPressed(event);
            }
            case KeyEvent.RELEASE -> {
                keyReleased(event);
                keyCodeMap.put(event.getKeyCode(), false);
            }
            case KeyEvent.TYPE ->
                keyTyped(event);
        }
     ...
     * pressed, but action keys such as Ctrl, Shift, and Alt are ignored.<br/>
     * <br/>
     * Because of how operating systems handle key repeats, holding down a key
     * may cause multiple calls to <b>keyTyped()</b>.The rate of repeat is set
     * by the operating system, and may be configured differently on each
     * computer.
     * <br />
     * <br />
     * Mouse and keyboard events only work when a program has <b>draw()</b>.
     ...
     * @webBrief Called once every time a key is pressed, but action keys such
     * as Ctrl, Shift, and Alt are ignored
     * @see UIFrame#keyPressed
     * @see UIFrame#key
     * @see UIFrame#keyCode
     * @see UIFrame#keyReleased()
     */
    ...
    //////////////////////////////////////////////////////////////
    public abstract void windowFocusGained();
...
//</editor-fold>
     ...
     * second. If the processor is not fast enough to maintain the specified
     * rate, the frame rate will not be achieved. Setting the frame rate within
    ...
    /**
     * Function for an application to kill itself and display an error. Mostly
     * this is here to be improved later.
     */
    ...
    public final void closeWindow() {
        ...
        surface.stopThread(); // Stop the rendering thread
        Frame frame = ((processing.awt.PSurfaceAWT.SmoothCanvas) surface.getNative()).getFrame();
        frame.dispose(); // Close the window without terminating the program
    }
    ...
    /**
     *
     * Saves an image from the display window. Append a file extension to the
     * name of the file, to indicate the file format to be used: either TIFF
     * (.tif), TARGA (.tga), JPEG (.jpg), or PNG (.png). If no extension is
     * included in the filename, the image will save in TIFF format and
     * <b>.tif</b> will be added to the name. These files are saved to the
     * sketch's folder, which may be opened by selecting "Show sketch folder"
     * from the "Sketch" menu. Alternatively, the files can be saved to any
     * location on the computer by using an absolute path (something that starts
     * with / on Unix and Linux, or a drive letter on Windows).<br />
     * <br />
     ...
     * @see UIFrame#saveFrame()
     * @see UIFrame#createGraphics(int, int, String)
     */
     ...
     * as
     * <b>mousePressed()</b> and <b>keyPressed()</b>. Use the Movie Maker
     * program in the Tools menu to combine these images to a movie.<br />
     * <br />
     ...
     * (####), which will be replaced by the current <b>frameCount</b> value.
     * (The number of hash marks is used to determine how many digits to include
     * in the file names.) Append a file extension, to indicate the file format
     * to be used: either TIFF (.tif), TARGA (.tga), JPEG (.jpg), or PNG (.png).
     * Image files are saved to the sketch's folder, which may be opened by
     ...
     * using an absolute path (something that starts with / on Unix and Linux,
     * or a drive letter on Windows).<br />
     * <br />
     * All images saved from the main drawing window will be opaque. To save
     * images without a background, use <b>createGraphics()</b>.
     *
     ...
     * function is run
     * @see UIFrame#save(String)
     * @see UIFrame#createGraphics(int, int, String, String)
     * @see UIFrame#frameCount
     ...
     * @see UIFrame#pow(float, float)
     * @see UIFrame#sq(float)
     */
     ...
     *
     * @webref math:calculation
     * @webBrief Calculates the natural logarithm (the base-<i>e</i> logarithm)
     * of a number
     ...
     *
     * @webref math:calculation
     * @webBrief Returns Euler's number <i>e</i> (2.71828...) raised to the
     * power of the
     * <b>value</b> parameter
     ...
     * @see UIFrame#min(float, float, float)
     */
...
//   */
  ...
  static public final double min(double a, double b) {
     ...
     * that value. <b>min()</b> accepts either two or three <b>float</b> or
     * <b>int</b>
     * values as parameters, or an array of any length.
     *
     * @webref math:calculation
     ...
     * @see UIFrame#max(float, float, float)
     */
 ...
 /*
  static public final double min(double[] list) {
     ...
     * @see UIFrame#cos(float)
     * @see UIFrame#tan(float)
     * @see UIFrame#radians(float)
     */
     ...
     * @see UIFrame#sin(float)
     * @see UIFrame#acos(float)
     * @see UIFrame#atan(float)
     */
     ...
     * parameter and the x-coordinate is the second due the structure of
     ...
     *
     * @webref math:trigonometry
     * @webBrief Converts a radian measurement to its corresponding value in
     * degrees
     ...
     *
     * @webref math:trigonometry
     * @webBrief Converts a degree measurement to its corresponding value in
     * radians
     * @param degrees degree value to convert to radians
     * @see UIFrame#degrees(float)
     */
     ...
     *
     * @webref math:calculation
     * @webBrief Calculates the closest int value that is greater than or equal
     * to the value of the parameter
     * @param n number to round up
     * @see UIFrame#floor(float)
     * @see UIFrame#round(float)
     */
     ...
     *
     * @webref math:calculation
     * @webBrief Calculates the closest int value that is less than or equal to
     * the value of the parameter
     * @param n number to round down
     * @see UIFrame#ceil(float)
     * @see UIFrame#round(float)
     */
     ...
     * @param c third value
     * @see UIFrame#dist(float, float, float, float)
     */
     ...
     *
     * @webref math:calculation
     * @webBrief Normalizes a number from another range into a value between 0
     * and 1
     * @param value the incoming value to be converted
     * @param start lower bound of the value's current range
     * @param stop upper bound of the value's current range
     * @see UIFrame#map(float, float, float, float, float)
     * @see UIFrame#lerp(float, float, float)
     */
     ...
     * window (0) to the right edge (width).<br />
     * <br />
     * As shown in the second example, numbers outside the range are not clamped
     * to the minimum and maximum parameters values, because out-of-range values
     ...
     * @see UIFrame#norm(float, float, float)
     * @see UIFrame#lerp(float, float, float)
     */
  ...
  static public final double map(double value,
     ...
     *
     * @webref math:random
     * @webBrief Returns a float from a random series of numbers having a mean
     * of 0 and standard deviation of 1
     * @see UIFrame#random(float, float)
     * @see UIFrame#noise(float, float, float)
     */
     ...
     * only one parameter is passed to the function, it will return a float
     * between zero and the value of the <b>high</b> parameter. For example,
     * <b>random(5)</b>
     ...
     * value between the two values. For example, <b>random(-5, 10.2)</b>
     * returns values starting at -5 and up to (but not including) 10.2. To
     * convert a floating-point random number to an integer, use the
     * <b>int()</b> function.
     *
     * @webref math:random
     ...
     * <b>seed</b>
     * parameter to a constant to return the same pseudo-random numbers each
    ...
    /**
     * Return a random integer from 0 up to (but not including) the specified
     * value for “high”. This is the same as calling random() and casting the
     * result to an <b>int</b>.
     */
    ...
    /**
     * Return a random integer from “low” up to (but not including) the
     * specified value for “high”. This is the same as calling random() and
     * casting the result to an <b>int</b>.
     */
     ...
     * a random sequence generator producing a more natural, harmonic succession
     * of numbers than that of the standard <b>random()</b> function. It was
     ...
     * <br />
     * In contrast to the <b>random()</b> function, Perlin noise is defined in
     * an infinite n-dimensional space, in which each pair of coordinates
     ...
     * noise space, as demonstrated in the first example above. The 2nd and 3rd
     * dimensions can also be interpreted as time.<br />
     * <br />
     ...
     * scale of the input coordinates. As the function works within an infinite
     * space, the value of the coordinates doesn't matter as such; only the
     * <em>distance</em> between successive coordinates is important (such as
     * when using <b>noise()</b> within a loop). As a general rule, the smaller
     * the difference between coordinates, the smoother the resulting noise
     * sequence. Steps of 0.005-0.03 work best for most applications, but this
     * will differ depending on use.<br />
     * <br />
     * There have been debates over the accuracy of the implementation of noise
     * in Processing. For clarification, it's an implementation of "classic
     * Perlin noise" from 1983, and not the newer "simplex noise" method from
     * 2001.
     *
     * @webref math:random
     ...
     * @see UIFrame#noiseSeed(long)
     * @see UIFrame#noiseDetail(int, float)
     * @see UIFrame#random(float, float)
     */
     ...
     * contributing exactly half than its predecessor, starting at 50% strength
     * for the first octave. This falloff amount can be changed by adding a
     * function parameter. For example, a falloff factor of 0.75 means each
     * octave will now have 75% impact (25% less) of the previous lower octave.
     * While any number between 0.0 and 1.0 is valid, note that values greater
     * than 0.5 may result in <b>noise()</b> returning values greater than
     * 1.0.<br />
     * <br />
     ...
     *
     * @webref math:random
     * @webBrief Adjusts the character and level of detail produced by the
     ...
     * directory of the current sketch.<br />
     * <br />
     ...
     * URL for a file found on a network.<br />
     * <br />
     ...
     * message does not halt the program, however the <b>null</b> value may
     * cause a NullPointerException if your code does not check whether the
     * value returned is <b>null</b>.<br />
     * <br />
     ...
     * shown in the third example on this page. Note that CMYK images are not
     * supported.<br />
     * <br />
    ...
    /**
     *
     * This function loads images on a separate thread so that your sketch
     * doesn't freeze while images load during <b>setup()</b>. While the image
     * is loading, its width and height will be 0. If an error occurs while
     * loading the image, its width and height will be set to -1. You'll know
     * when the image has loaded properly because its <b>width</b> and
     * <b>height</b> will be greater than 0. Asynchronous image loading
     * (particularly when downloading from a server) can dramatically improve
     * performance.<br />
     * <br />
     * The <b>extension</b> parameter is used to determine the image type in
     * cases where the image filename does not end with a proper extension.
     ...
     * "jpg"
     * @see PImage
     * @see UIFrame#loadImage(String, String)
     */
    ...
    // DATA I/O
    /**
     * Reads the contents of a file or URL and creates an XML object with its
     * values. If a file is specified, it must be located in the sketch's "data"
     * folder. The filename parameter can also be a URL to a file found
     * online.<br /><br />
     * All files loaded and saved by the Processing API use UTF-8 encoding. If
     * you need to load an XML file that's not in UTF-8 format, see the
     ...
     * @see XML
     * @see UIFrame#parseXML(String)
     * @see UIFrame#saveXML(XML, String)
     * @see UIFrame#loadBytes(String)
     * @see UIFrame#loadStrings(String)
     * @see UIFrame#loadTable(String)
     */
     ...
     * String does not contain XML data or cannot be parsed, a <b>null</b> value
     * is returned.<br />
     * <br />
     ...
     * performing operations on the new XML object, in case the String content
     ...
     * <b>JSONObject</b>. If the <b>String</b> does not contain
     * <b>JSONObject</b>
     * data or cannot be parsed, a <b>null</b> value is returned.<br />
     * <br />
     * <b>parseJSONObject()</b> is most useful when pulling data dynamically,
     * such as from third-party APIs. Normally, API results would be saved to a
     * <b>String</b>, and then can be converted to a structured
     * <b>JSONObject</b>
     * using <b>parseJSONObject()</b>. Be sure to check if <b>null</b> is
     * returned before performing operations on the new <b>JSONObject</b> in
     * case the
     * <b>String</b> content could not be parsed.<br />
     * <br />
     * If your data already exists as a <b>JSON</b> file in the data folder, it
     * is simpler to use <b>loadJSONObject()</b>.
     *
     * @webref input:files
     * @webBrief Takes a <b>String</b>, parses its contents, and returns a
     * <b>JSONObject</b>
     * @param input String to parse as a JSONObject
     * @see UIFrame#loadJSONObject(String)
     * @see UIFrame#saveJSONObject(JSONObject, String)
     */
     ...
     * @see JSONObject
     * @see JSONArray
     * @see UIFrame#loadJSONArray(String)
     * @see UIFrame#saveJSONObject(JSONObject, String)
     * @see UIFrame#saveJSONArray(JSONArray, String)
     */
    ...
    /**
     * Takes a <b>String</b>, parses its contents, and returns a
     * <b>JSONArray</b>. If the <b>String</b> does not contain <b>JSONArray</b>
     * data or cannot be parsed, a <b>null</b> value is returned.<br />
     * <br />
     ...
     * using <b>parseJSONArray()</b>. Be sure to check if <b>null</b> is
     * returned before performing operations on the new <b>JSONArray</b> in case
     * the
     * <b>String</b> content could not be parsed.<br />
     * <br />
     * If your data already exists as a <b>JSON</b> file in the data folder, it
    ...
    /**
     * Loads an array of JSON objects from the data folder or a URL, and returns
     * a
     * <b>JSONArray</b>. Per standard JSON syntax, the array must be enclosed in
     * a pair of hard brackets <b>[]</b>, and each object within the array must
     ...
     * tab-separated data, include "tsv" in the options parameter if the
     * filename or URL does not end in <b>.tsv</b>. Note: If an extension is in
     * both places, the extension in the <b>options</b> is used.<br />
     * <br />
     ...
     * "header" option.<br />
     * <br />
     * Some CSV files contain newline (CR or LF) characters inside cells. This
     * is rare, but adding the "newlines" option will handle them properly.
     * (This is not enabled by default because the parsing code is much
     * slower.)<br />
     * <br />
     ...
     * @param filename name of a file in the data folder or a URL.
     * @see Table
     * @see UIFrame#saveTable(Table, String)
     * @see UIFrame#loadBytes(String)
     * @see UIFrame#loadStrings(String)
     * @see UIFrame#loadXML(String)
     */
...
//    }
    ...
    /**
     *
     * Loads a .vlw formatted font into a <b>PFont</b> object. Create a .vlw
     * font by selecting "Create Font..." from the Tools menu. This tool creates
     * a texture for each alphanumeric character and then adds them as a .vlw
     * file to the current sketch's data folder. Because the letters are defined
     * as textures (and not vector data) the size at which the fonts are created
     * must be considered in relation to the size at which they are drawn. For
     * example, load a 32pt font if the sketch displays the font at 32 pixels or
     * smaller. Conversely, if a 12pt font is loaded and displayed at 48pts, the
     * letters will be distorted because the program will be stretching a small
     * graphic to a large size.<br />
     * <br />
     ...
     * re-loaded from the disk (or network) on each frame. It's recommended to
     ...
     * the local computer using an absolute path (something that starts with /
     * on Unix and Linux, or a drive letter on Windows), or the filename
     ...
     * data to be used with the default renderer setting. This can be helpful
     * when many font sizes are needed, or when using any renderer based on the
     * default renderer, such as the PDF library.
     *
     ...
     * @see UIFrame#createFont(String, float, boolean, char[])
     */
    ...
    /**
     *
     * Dynamically converts a font to the format used by Processing from a .ttf
     * or .otf file inside the sketch's "data" folder or a font that's installed
     ...
     * function. Not all fonts can be used and some might work with one
     * operating system and not others. When sharing a sketch with other people
     * or posting it on the web, you may need to include a .ttf or .otf version
     * of your font in the data directory of the sketch because other people
     * might not have the font installed on their computer. Only fonts that can
     * legally be distributed should be included with a sketch.<br />
     * <br />
     * The <b>size</b> parameter states the font size you want to generate. The
     * <b>smooth</b> parameter specifies if the font should be anti-aliased or
     * not. The <b>charset</b> parameter is an array of chars that specifies the
     ...
     * rendered quickly. In the <b>P2D</b> and <b>P3D</b> renderers, the
     * function sets the project to render the font as a series of small
     * textures. For instance, when using the default renderer, the actual
     * native version of the font will be employed by the sketch, improving
     * drawing quality and performance. With the <b>P2D</b> and <b>P3D</b>
     * renderers, the bitmapped version will be used to improve speed and
     * appearance, but the results are poor when exporting if the sketch does
     * not include the .otf or .ttf file, and the requested font is not
     ...
     * 'callback' function. If the dialog is closed or canceled, <b>null</b>
     ...
     *
     * @webref input:files
     * @webBrief Open a platform-specific file chooser dialog to select a file
     * for input
     ...
     * output. After the selection is made, the selected File will be passed to
     * the 'callback' function. If the dialog is closed or canceled, <b>null</b>
     * will be sent to the function, so that the program is not waiting for
     * additional input. The callback is necessary because of how threading
     * works.
     *
     * @webref output:files
     * @webBrief Opens a platform-specific file chooser dialog to select a file
     * for output
     * @param prompt message to the user
     * @param callback name of the method to be called when the selection is
     * made
    ...
    /**
     * Opens a platform-specific file chooser dialog to select a folder. After
     * the selection is made, the selection will be passed to the 'callback'
     * function. If the dialog is closed or canceled, null will be sent to the
     * function, so that the program is not waiting for additional input. The
     * callback is necessary because of how threading works.
     *
     * @webref input:files
     * @webBrief Opens a platform-specific file chooser dialog to select a
     * folder
     * @param prompt message to the user
     * @param callback name of the method to be called when the selection is
     * made
     ...
     * to the <b>createWriter()</b> function. For more information about the
     ...
     * @see BufferedReader
     * @see UIFrame#createWriter(String)
     * @see PrintWriter
     */
     ...
     * <b>PrintWriter</b> object to write to it
     * @param filename name of the file to be created
     * @see PrintWriter
     * @see UIFrame#createReader
     * @see BufferedReader
     */
     ...
     * application)<br />
     * <br />
     * If the requested item doesn't exist, <b>null</b> is returned. If not
     * online, this will also check to see if the user is asking for a file
     * whose name isn't properly capitalized. If capitalization is different, an
     * error will be printed to the console. This helps prevent issues that
     * appear when a sketch is exported to the web, where case sensitivity
     * matters, as opposed to running from inside the Processing Development
     * Environment on Windows or macOS, where case sensitivity is preserved but
     * ignored.<br />
     * <br />
     ...
     * UIFrame to easily open things from the data folder or from a URL, but
     ...
     * die() would be called, killing the sketch)
     * <p>
     ...
     * opposed to Windows and the macOS default where case sensitivity is
     ...
     * @see UIFrame#createOutput(String)
     * @see UIFrame#selectOutput(String, String)
     * @see UIFrame#selectInput(String, String)
     *
     */
    ...
    /**
     *
     * Reads the contents of a file and places it in a byte array. If the name
     * of the file is used as the parameter, as in the above example, the file
     * must be loaded in the sketch's "data" directory/folder. <br />
     * <br />
     * Alternatively, the file maybe be loaded from anywhere on the local
     * computer using an absolute path (something that starts with / on Unix and
     * Linux, or a drive letter on Windows), or the filename parameter can be a
     ...
     * @param filename name of a file in the data folder or a URL.
     * @see UIFrame#loadStrings(String)
     * @see UIFrame#saveStrings(String, String[])
     * @see UIFrame#saveBytes(String, byte[])
     *
     */
     ...
     * the function, which is more appropriate for "sketch" projects. <br />
     * <br />
     ...
     * @see UIFrame#createInput(String)
     * @see UIFrame#selectOutput(String, String)
     */
     ...
     * <br />
     * The <b>target</b> parameter can be either a String specifying a file
     * name, or, for greater control over the file location, a <b>File</b>
     * object. (Note that, unlike some other functions, this will not
     * automatically compress or uncompress gzip files.)
     *
     * @webref output:files
     ...
     * array of bytes to a file. The data is saved in binary format. This file
     * is saved to the sketch's folder, which is opened by selecting "Show
     * Sketch Folder" from the "Sketch" menu. Alternatively, the files can be
     * saved to any location on the computer by using an absolute path
     * (something that starts with / on Unix and Linux, or a drive letter on
     * Windows).
     *
     * @webref output:files
    ...
    /**
     *
     * Writes an array of Strings to a file, one line per String. By default,
     * this file is saved to the sketch's folder. This folder is opened by
     * selecting "Show Sketch Folder" from the "Sketch" menu.<br />
     * <br />
     * Alternatively, the file can be saved to any location on the computer by
     ...
     * Processing API use UTF-8 encoding. In earlier releases, the default
     * encoding for your platform was used, which causes problems when files are
     * moved to other platforms.
     *
     * @webref output:files
    ...
    static protected String calcSketchPath() {
        ...
        try {
            ...
            URL jarURL
                    = UIFrame.class.getProtectionDomain().getCodeSource().getLocation();
            // Decode URL
     ...
     * meaning that init() was never called on the UIFrame when hosted my some
     * other main() or by other code. For proper use of init(), see the examples
     * in the main description text for UIFrame.
     */
     ...
     * jar file of the exported application. In this case, it's not possible to
     * save data into the jar file, because it will often be running from a
     * server, or marked in-use if running from a local file system. With this
     * in mind, saving to the data path doesn't make sense anyway. If you know
     ...
     * dst)</b> &mdash; copies an entire array to another of the same size. It
     * is equivalent to <b>arrayCopy(src, 0, dst, 0, src.length)</b>.<br />
     * <br />
     ...
     * brackets). If used with a two (or three or more) dimensional array, it
     * will only copy the references at the first level, because a
     * two-dimensional array is simply an "array of arrays". This does not
     * produce an error, however, because this is often the desired behavior.
     * Internally, this function calls Java's <a href=
     ...
     * @see UIFrame#concat(boolean[], boolean[])
     */
     ...
     *
     * @webref data:array functions
     * @webBrief Expands an array by one element and adds data to the new
     * position
     * @param array array to append
     * @param value new data for the array
     * @see UIFrame#shorten(boolean[])
     * @see UIFrame#expand(boolean[])
     */
     ...
     *
     * @webref data:array functions
     * @webBrief Decreases an array by one element and returns the shortened
     * array
     * @param list array to shorten
     * @see UIFrame#append(byte[], byte)
     * @see UIFrame#expand(boolean[])
     */
     ...
     * two parameters must be arrays of the same datatype. The first parameter
     ...
     * that array index numbering starts at zero, so the first position is 0,
     * the second position is 1, and so on.)<br />
     * <br />
     * When splicing an array of objects, the data returned from the function
     * must be cast to the object array's data type. For example:
     * <em>SomeClass[] items = (SomeClass[]) splice(array1, array2, index)</em>
     *
     * @webref data:array functions
     ...
     * @see UIFrame#concat(boolean[], boolean[])
     * @see UIFrame#subset(boolean[], int, int)
     */
     ...
     * the
     * <b>start</b> and <b>count</b> parameters specify which elements to
     * extract. If no <b>count</b> is given, elements will be extracted from the
     ...
     * @see UIFrame#splice(boolean[], boolean, int)
     */
     ...
     * @see UIFrame#splice(boolean[], boolean, int)
     * @see UIFrame#arrayCopy(Object, int, Object, int, int)
     */
     ...
     * @see UIFrame#split(String, String)
     * @see UIFrame#join(String[], char)
     */
     ...
     *
     * @webref data:string_functions
     * @webBrief Combines an array of <b>Strings</b> into one <b>String</b>,
     * each separated by the character(s) used for the <b>separator</b>
     * parameter
     * @param list array of Strings
     * @param separator char or String to be placed between each item
     * @see UIFrame#split(String, String)
     * @see UIFrame#trim(String)
     * @see UIFrame#nf(float, int, int)
     * @see UIFrame#nfs(float, int, int)
     */
     ...
     * character delimiters or "tokens". The <b>delim</b> parameter specifies
     * the character or characters to be used as a boundary.<br />
     * <br />
     ...
     *
     * @webref data:string_functions
     * @webBrief The <b>splitTokens()</b> function splits a <b>String</b> at one
     * or many character "tokens"
     * @param value the String to be split
     * @param delim list of individual characters that will be used as
     * separators
     * @see UIFrame#split(String, String)
     * @see UIFrame#join(String[], String)
     * @see UIFrame#trim(String)
     */
     ...
     * two backslashes (\\\\) in front of the character (see example above). You
     * can read more about
     ...
     * String array. If there are no matches, a <b>null</b> value will be
     * returned. If no groups are specified in the regular expression, but the
     * sequence matches, an array of length 1 (with the matched text as the
     * first element of the array) will be returned.<br />
     * <br />
     * To use the function, first check to see if the result is <b>null</b>. If
     * the result is null, then the sequence did not match at all. If the
     * sequence did match, an array is returned.<br />
     * <br />
     ...
     *
     * @webref data:string_functions
     * @webBrief The function is used to apply a regular expression to a piece
     * of text, and return matching groups (elements found inside parentheses)
     * as a <b>String</b> array
     * @param str the String to be searched
     * @param regexp the regexp to be used for matching
     * @see UIFrame#matchAll(String, String)
     * @see UIFrame#split(String, String)
     * @see UIFrame#splitTokens(String, String)
     * @see UIFrame#join(String[], String)
     * @see UIFrame#trim(String)
     */
     ...
     * To use the function, first check to see if the result is <b>null</b>. If
     * the result is null, then the sequence did not match at all. If the
     * sequence did match, a 2D array is returned.<br />
     * <br />
     * If there are groups (specified by sets of parentheses) in the regular
     * expression, then the contents of each will be returned in the array.
     ...
     *
     * @webref data:string_functions
     * @webBrief This function is used to apply a regular expression to a piece
     * of text
     * @param str the String to be searched
     * @param regexp the regexp to be used for matching
     * @see UIFrame#match(String, String)
     * @see UIFrame#split(String, String)
     * @see UIFrame#splitTokens(String, String)
     * @see UIFrame#join(String[], String)
     * @see UIFrame#trim(String)
     ...
     * used when the number is invalid. If there's a decimal place, it will be
     * truncated, making this more of a toInt() than parseInt() function. This
     * is because the method is used internally for casting. Not ideal, but the
     * name was chosen before that clarification was made.
     */
     ...
     * positive integers. The <b>left</b> parameter should be positive or 0. If
     * it is zero, only the right side is formatted.<br />
     * <br />
     * As shown in the above example, <b>nf()</b> is used to add zeros to the
     * left and/or right of a number. This is typically for aligning a list of
     * numbers. To <em>remove</em> digits from a floating-point number, use the
     ...
     * @see UIFrame#nfs(float, int, int)
     * @see UIFrame#nfp(float, int, int)
     * @see UIFrame#nfc(float, int)
     * @see <a href=
     ...
     * appropriate commas to mark units of 1000. There are four versions: one
     * for formatting ints, one for formatting an array of ints, one for
     * formatting floats, and one for formatting an array of floats.<br />
     * <br />
     ...
     * whatever is appropriate for that region.
     *
     * @webref data:string_functions
     * @webBrief Utility function for formatting numbers into strings and
     ...
     * one for formatting ints. The values for the <b>digits</b>, <b>left</b>,
     * and
     ...
     * <b>String</b> containing the equivalent hexadecimal notation. For
     * example, the <b>color</b> value produced by <b>color(0, 102, 153)</b>
     * will convert to the <b>String</b> value <b>"FF006699"</b>. This function
     * can help make your geeky debugging sessions much happier.<br />
     * <br />
     ...
     * @webBrief Converts a <b>byte</b>, <b>char</b>, <b>int</b>, or
     * <b>color</b> to a <b>String</b> containing the equivalent hexadecimal
     * notation
     * @param value the value to convert
     * @see UIFrame#unhex(String)
     * @see UIFrame#binary(byte)
     * @see UIFrame#unbinary(String)
     */
     ...
     *
     * @webref data:conversion
     * @webBrief Converts a <b>String</b> representation of a hexadecimal number
     * to its equivalent integer value
     * @param value String to convert to an integer
     * @see UIFrame#hex(int, int)
     * @see UIFrame#binary(byte)
     * @see UIFrame#unbinary(String)
     */
   ...
   * The digits parameter determines how many digits will be used.
     ...
     * <b>color</b> value produced by <b>color(0, 102, 153, 255)</b> will
     * convert to the <b>String</b> value
     * <b>"11111111000000000110011010011001"</b>. This function can help make
     * your geeky debugging sessions much happier.<br />
     * <br />
     * Note that the maximum number of digits is 32, because an <b>int</b> value
     * can only represent up to 32 bits. Specifying more than 32 digits will
     * have no effect.
     *
     * @webref data:conversion
     * @webBrief Converts an <b>int</b>, <b>byte</b>, <b>char</b>, or
     * <b>color</b> to a
    ...
    /**
     *
     * Converts a <b>String</b> representation of a binary number to its
     * equivalent integer value. For example, <b>unbinary("00001000")</b> will
     * return
     * <b>8</b>.
     *
     * @webref data:conversion
     * @webBrief Converts a <b>String</b> representation of a binary number to
     * its equivalent <b>integer</b> value
     * @param value String to convert to an integer
     * @see UIFrame#binary(byte)
     * @see UIFrame#hex(int, int)
     * @see UIFrame#unhex(String)
     ...
     * therefore, <b>color(255, 204, 0)</b> will return a bright yellow color
     * (see the first example above).<br />
     * <br />
    ...
    /**
     *
     * Calculates a new <b>color</b> that is a blend of two other colors. The
     * <b>amt</b> parameter controls the amount of each color to use where an
     * amount of 0.0 will produce the first color, 1.0 will return the second
     * color, and 0.5 is halfway in between. Values between 0.0 and 1.0 will
     * interpolate between the two colors in that proportion. <br />
     ...
     * <b>colors</b> at a specific increment
     * @usage web_application
  ...
  public void frameResized(int w, int h) {
    ...
    /**
     * Internal use only: called by Surface objects to queue a resize event to
     * call windowResized() when it's safe, which is after the beginDraw() call
     * and before the draw(). Note that this is only the notification that the
     * resize has happened.
     */
    ...
    /**
     * When running from the PDE, this saves the window position for next time
     * the sketch is run. Needs to remain a separate method so that it can be
     * overridden by Python Mode.
     */
    ...
    /**
     * Internal use only: called by Surface objects to queue a position event to
     * call windowPositioned() when it's safe, which is after the beginDraw()
     * call and before the draw(). Note that this is only the notification that
     * the window is in a new position.
     */
    ...
    /**
     * Scale the sketch as if it fits this specific width and height. This will
     * also scale the mouseX and mouseY variables (as well as pmouseX and
     * pmouseY). Note that it will not have an effect on MouseEvent objects
     * (i.e. event.getX() and event.getY()) because their exact behavior may
     * interact strangely with other libraries.
     */
     ...
     * <p>
     * Usage: UIFrame [options] &lt;class name&gt; [sketch args]
     * <ul>
     ...
     * the sketch itself. These will be passed into the args[] array in UIFrame.
     * <p>
    ...
    /**
     * Convenience method so that UIFrame.main(YourSketch.class) launches a
    ...
    /**
     * Convenience method so that UIFrame.main("YourSketch") launches a sketch,
    ...
    /**
     * Convenience method so that UIFrame.main("YourSketch", args) launches a
     ...
     * understood by) UIFrame such as --display.
     */
    ...
    static public void runSketch(final String[] args,
            final UIFrame constructedSketch) {
      ...
      if (coreJarPath.endsWith("/Contents/Java/core.jar")) {
        ...
        if (constructedSketch != null) {
            ...
            try {
                ...
                sketch = (UIFrame) c.getDeclaredConstructor().newInstance();
        ...
        if (platform == MACOS && !disableAWT) {
            try {
                ...
                Method method
                        = thinkDifferent.getMethod("init", UIFrame.class);
        ...
        if (args.length != argIndex + 1) {
            ...
            sketch.args = UIFrame.subset(args, argIndex + 1);
        }
 ...
 /*
    ...
    static public void hideMenuBar() {
        if (platform == MACOS) {
            ...
            try {
                final String td = "processing.core.ThinkDifferent";
                final Class<?> thinkDifferent = UIFrame.class.getClassLoader().loadClass(td);
    ...
    /**
     * Get the display scaling for Windows by calling out to a helper app. More
     * <a href="https://github.com/processing/processing4/tree/master/build/windows/fenster">here</a>.
     */
     ...
     * the recording process and close the file. <br />
     * <br />
     ...
     *
     * @webref output:files
     * @webBrief Opens a new file and all subsequent drawing functions are
     * echoed to this file as well as the display window
     * @param renderer PDF or SVG
     * @param filename filename for output
     * @see UIFrame#endRecord()
     */
     ...
     * closes the file
     * @see UIFrame#beginRecord(String, String)
     */
     ...
     *
     * @webref output:files
     * @webBrief Complement to <b>beginRaw()</b>; they must always be used
     * together
     * @see UIFrame#beginRaw(String, String)
     */
    public void endRaw() {
        g.endRaw();
    }
    ...
    /**
     *
     * Loads the pixel data of the current display window into the
     * <b>pixels[]</b>
     ...
     * array that's part of the UIFrame object. Allows the use of pixels[] in
     * the code, rather than g.pixels[].
     *
     * @webref image:pixels
     ...
     * &mdash; updating is only necessary to apply changes.
     *
     * @webref image:pixels
     ...
     * the <b>kind</b> parameter tells it which types of shapes to create from
     ...
     * After calling the <b>beginShape()</b>
     * function, a series of <b>vertex()</b> commands must follow. To stop
     * drawing the shape, call <b>endShape()</b>. The <b>vertex()</b> function
     * with two parameters specifies a position in 2D and the <b>vertex()</b>
     * function with three parameters specifies a position in 3D. Each shape
     * will be outlined with the current stroke color and filled with the fill
     * color. <br />
     * <br />
     ...
     * altered on a per-vertex basis, but the default renderer does not.
     * Settings such as <b>strokeWeight()</b>, <b>strokeCap()</b>, and
     * <b>strokeJoin()</b>
     * cannot be changed while inside a <b>beginShape()</b>/<b>endShape()</b>
     * block with any renderer.
     *
     * @webref shape:vertex
     * @webBrief Using the <b>beginShape()</b> and <b>endShape()</b> functions
     * allow creating more complex forms
    ...
    /**
     *
     * Sets the current normal vector. Used for drawing three-dimensional shapes
     * and surfaces, <b>normal()</b> specifies a vector perpendicular to a
     ...
     * <br />
     * With <b>IMAGE</b>, if an image is 100 x 200 pixels, mapping the image
     * onto the entire size of a quad would require the points (0,0) (100, 0)
     * (100,200) (0,200). The same mapping in <b>NORMAL</b> is (0,0) (1,0) (1,1)
     * (0,1).
     *
     ...
     * and before any calls to <b>vertex()</b>. This function only works with
     ...
     * lines, triangles, quads, and polygons. It is used exclusively within the
    ...
    /**
     * Use the <b>beginContour()</b> and <b>endContour()</b> function to create
     * negative shapes within shapes such as the center of the letter "O".
     * <b>beginContour()</b> begins recording vertices for the shape and
     * <b>endContour()</b> stops recording. The vertices that define a negative
     * shape must "wind" in the opposite direction from the exterior shape.
     * First draw vertices for the exterior shape in clockwise order, then for
     * internal shapes, draw vertices counterclockwise.<br />
     * <br />
     ...
     * <b>scale()</b> do not work within a
     * <b>beginContour()</b>/<b>endContour()</b>
     * pair. It is also not possible to use other shapes, such as
     * <b>ellipse()</b>
     * or <b>rect()</b> within.
     *
     * @webref shape:vertex
     ...
     *
     * @webref shape:vertex
     * @webBrief the companion to <b>beginShape()</b> and may only be called
     * after <b>beginShape()</b>
    ...
    /**
     * Loads geometry into a variable of type <b>PShape</b>. SVG and OBJ files
     * may be loaded. To load correctly, the file must be located in the data
     * directory of the current sketch. In most cases,
    ...
    /**
     * The <b>createShape()</b> function is used to define a new shape. Once
     * created, this shape can be drawn with the <b>shape()</b>
     * function. The basic way to use the function defines new primitive shapes.
     * One of the following parameters are used as the first parameter:
     * <b>ELLIPSE</b>, <b>RECT</b>, <b>ARC</b>, <b>TRIANGLE</b>,
     * <b>SPHERE</b>, <b>BOX</b>, <b>QUAD</b>, or <b>LINE</b>. The parameters
     * for each of these different shapes are the same as their corresponding
     * functions: <b>ellipse()</b>, <b>rect()</b>, <b>arc()</b>,
     * <b>triangle()</b>, <b>sphere()</b>, <b>box()</b>, <b>quad()</b>, and
     * <b>line()</b>. The first example above clarifies how this works.<br />
     * <br />
     * Custom, unique shapes can be made by using <b>createShape()</b> without a
     * parameter. After the shape is started, the drawing attributes and
     * geometry can be set directly to the shape within the <b>beginShape()</b>
     * and <b>endShape()</b> methods. See the second example above for
     * specifics, and the reference for <b>beginShape()</b> for all of its
     * options.<br />
     * <br />
     ...
     * calling methods like <b>setFill()</b> and <b>setStroke()</b>, as seen in
     * the examples above. The complete list of methods and fields for the
     ...
     * loaded in the sketch's "data" folder/directory to load correctly. Shaders
     * are compatible with the P2D and P3D renderers, but not with the default
     * renderer.<br />
     * <br />
     * Alternatively, the file maybe be loaded from anywhere on the local
     * computer using an absolute path (something that starts with / on Unix and
    ...
    /**
     *
     * Applies the shader specified by the parameters. It's compatible with the
     ...
     * <b>imageMode()</b> function, either CORNER, CORNERS, or CENTER.
     *
     * @webref rendering
     ...
     * <br />
     * BLEND - linear interpolation of colors: <b>C = A*factor + B</b>. This is
     * the default.<br />
     * <br />
     ...
     * 0)</b><br />
     * <br />
     * DARKEST - only the darkest color succeeds: <b>C = min(A*factor,
     * B)</b><br />
     * <br />
     ...
     *
     * @webref rendering
     * @webBrief Blends the pixels in the display window according to a defined
     * mode
     ...
     * <b>quadraticVertex()</b> defines the position of one control point and
     * one anchor point of a Bézier curve, adding a new segment to a line or
     * shape. The first time <b>quadraticVertex()</b> is used within a
     * <b>beginShape()</b> call, it must be prefaced with a call to
     * <b>vertex()</b> to set the first anchor point. This function must be used
     * between <b>beginShape()</b> and <b>endShape()</b> and only when there is
     * no MODE parameter specified to <b>beginShape()</b>. Using the 3D version
     ...
     * the beginning and end of the curve. A minimum of four points is required
     * to draw a tiny curve between the second and third points. Adding a fifth
     * point with <b>curveVertex()</b> will draw the curve between the second,
     * third, and fourth points. The <b>curveVertex()</b>
     ...
     * <br />
     * Point appears round with the default <b>strokeCap(ROUND)</b> and square
     * with
     * <b>strokeCap(PROJECT)</b>. Points are invisible with
     * <b>strokeCap(SQUARE)</b>
     * (no cap). <br />
     * <br />
     * Using point() with strokeWeight(1) or smaller may draw nothing to the
     * screen, depending on the graphics settings of the computer. Workarounds
     * include setting the pixel using <b>set()</s> or drawing the point using
     * either
     * <b>circle()</b> or <b>square()</b>.
     *
     * @webref shape:2d primitives
     * @webBrief Draws a point, a coordinate in space at the dimension of one
     * pixel
     * @param x x-coordinate of the point
     * @param y y-coordinate of the point
     ...
     * in which parameters given to <b>rect()</b> are interpreted.<br />
     * <br />
     * The default mode is <b>rectMode(CORNER)</b>, which interprets the first
     * two parameters of <b>rect()</b> as the upper-left corner of the shape,
     * while the third and fourth parameters are its width and height.<br />
     * <br />
     * <b>rectMode(CORNERS)</b> interprets the first two parameters of
     * <b>rect()</b>
     * as the location of one corner, and the third and fourth parameters as the
     * location of the opposite corner.<br />
     * <br />
     * <b>rectMode(CENTER)</b> interprets the first two parameters of
     * <b>rect()</b>
     * as the shape's center point, while the third and fourth parameters are
     * its width and height.<br />
     * <br />
     * <b>rectMode(RADIUS)</b> also uses the first two parameters of
     * <b>rect()</b>
     ...
     * fourth sets the height. The way these parameters are interpreted,
     * however, may be changed with the <b>rectMode()</b> function.<br />
     * <br />
     * To draw a rounded rectangle, add a fifth parameter, which is used as the
     * radius value for all four corners.<br />
     * <br />
     * To use a different radius value for each corner, include eight
     * parameters. When using eight parameters, the latter four set the radius
     * of the arc at each corner separately, starting with the top-left corner
     * and moving clockwise around the rectangle.
     *
     * @webref shape:2d primitives
     ...
     * in which parameters given to <b>ellipse()</b> are interpreted.<br />
     * <br />
     * The default mode is <b>ellipseMode(CENTER)</b>, which interprets the
     * first two parameters of <b>ellipse()</b> as the shape's center point,
     * while the third and fourth parameters are its width and height.<br />
     * <br />
     * <b>ellipseMode(RADIUS)</b> also uses the first two parameters of
     * <b>ellipse()</b> as the shape's center point, but uses the third and
     * fourth parameters to specify half of the shape's width and height.<br />
     * <br />
     * <b>ellipseMode(CORNER)</b> interprets the first two parameters of
     * <b>ellipse()</b> as the upper-left corner of the shape, while the third
     * and fourth parameters are its width and height.<br />
     * <br />
     * <b>ellipseMode(CORNERS)</b> interprets the first two parameters of
     * <b>ellipse()</b> as the location of one corner of the ellipse's bounding
     * box, and the third and fourth parameters as the location of the opposite
     * corner.<br />
     * <br />
     * The parameter must be written in ALL CAPS because Processing is a
     * case-sensitive language.
     *
     * @webref shape:attributes
     * @webBrief The origin of the ellipse is modified by the
     * <b>ellipseMode()</b>
     * function
     * @param mode either CENTER, RADIUS, CORNER, or CORNERS
     * @see UIFrame#ellipse(float, float, float, float)
     * @see UIFrame#arc(float, float, float, float, float, float)
     */
     ...
     * height. The origin may be changed with the <b>ellipseMode()</b> function.
     *
     * @webref shape:2d primitives
     ...
     * ellipse defined by the <b>a</b>, <b>b</b>, <b>c</b>, and <b>d</b>
     ...
     * angles (in radians) at which to draw the arc. The start/stop values must
     * be in clockwise order. <br />
     * <br />
     * There are three ways to draw an arc; the rendering technique used is
     * defined by the optional seventh parameter. The three options, depicted in
     * the above examples, are PIE, OPEN, and CHORD. The default mode is the
     * OPEN stroke with a PIE fill. <br />
     * <br />
     * In some cases, the <b>arc()</b> function isn't accurate enough for smooth
     * drawing. For example, the shape may jitter on screen when rotating
     * slowly. If you're having an issue with how arcs are rendered, you'll need
     * to draw the arc yourself with <b>beginShape()</b>/<b>endShape()</b> or a
     * <b>PShape</b>.
     *
     * @webref shape:2d primitives
   ...
   * @param mode either OPEN, CHORD, or PIE
    ...
    /**
     * A box is an extruded <b>rectangle</b>. A box with equal dimension on all
     * sides is a cube.
     *
     ...
     * @webref shape:3d primitives
     * @webBrief Controls the detail used to render a sphere by adjusting the
     * number of vertices of the sphere mesh
     * @see PGraphics#sphere(float)
     */
    ...
    /**
     * Sets the resolution at which Bézier curves display. The default value is
     * 20. This function is only useful when using the <b>P2D</b> or <b>P3D</b>
     * renderer; the default (JAVA2D) renderer does not use this information.
     *
     * @webref shape:curves
     ...
     * of the curve. The curves were developed by French engineer Pierre Bezier.
     * Using the 3D version requires rendering with P3D (see the Environment
     * reference for more information).
     *
     * <h3>Advanced</h3>
     * Draw a cubic Bézier curve. The first and last points are the on-curve
     ...
     * <b>c</b>, <b>d</b>. The parameter <b>t</b> may range from 0 (the start of
     * the curve) and 1 (the end of the curve). <b>a</b> and <b>d</b> are the
     * control points, and <b>b</b> and <b>c</b> are points on the curve. As
     * seen in the example above, this can be used once with the <b>x</b>
     * coordinates and a second time with the <b>y</b> coordinates to get the
     * location of a curve at
     * <b>t</b>.
     *
     * @webref shape:curves
     ...
     * <b>curveVertex()</b>. The parameter <b>tightness</b> determines how the
     * curve fits to the vertex points. The value 0.0 is the default value for
     * <b>tightness</b> (this value defines the curves to be Catmull-Rom
     * splines) and the value 1.0 connects all the points with straight lines.
     * Values within the range -5.0 and 5.0 will deform the curves but will
     * leave them recognizable and as values increase in magnitude, they will
     * continue to deform.
     *
     * @webref shape:curves
     ...
     * <br />
     * The default mode is <b>imageMode(CORNER)</b>, which interprets the second
     * and third parameters of <b>image()</b> as the upper-left corner of the
     * image. If two additional parameters are specified, they are used to set
     * the image's width and height.<br />
     * <br />
     ...
     * <b>image()</b> as the image's center point. If two additional parameters
     * are specified, they are used to set the image's width and height.<br />
     * <br />
     * The parameter must be written in ALL CAPS because Processing is a
     * case-sensitive language.
     *
    ...
    /**
     *
     * The <b>image()</b> function draws an image to the display window. Images
     * must be in the sketch's "data" directory to load correctly. Select "Add
     * file..." from the "Sketch" menu to add the image to the data directory,
     * or just drag the image file onto the sketch window. Processing currently
     * works with GIF, JPEG, and PNG images. <br />
     * <br />
     * The <b>img</b> parameter specifies the image to display and by default
     * the
     * <b>a</b> and <b>b</b> parameters define the location of its upper-left
     * corner. The image is displayed at its original size unless the <b>c</b>
     * and
     * <b>d</b> parameters specify a different size. The <b>imageMode()</b>
     * function can be used to change the way these parameters draw the
     * image.<br />
     * <br />
     ...
     * to add the shape. Processing currently works with SVG, OBJ, and
     * custom-created shapes. The <b>shape</b> parameter specifies the shape to
     * display and the coordinate parameters define the location of the shape
     * from its upper-left corner. The shape is displayed at its original size
     * unless the <b>c</b> and
     * <b>d</b> parameters specify a different size. The <b>shapeMode()</b>
     ...
     * the values for the <b>x</b> and <b>y</b> parameters of the <b>text()</b>
     * function. <br />
     * <br />
     ...
     * parameters are straightforward. The BOTTOM parameter offsets the line
     * based on the current
     * <b>textDescent()</b>. For multiple lines, the final line will be aligned
     * to the bottom, with the previous lines appearing above it. <br />
     * <br />
     ...
     * useful drawing mode for text drawn in a rectangle.) <br />
     * <br />
     ...
     * To do this as less of a hack, use some percentage of <b>textAscent()</b>
     * or
     * <b>textDescent()</b> so that the hack works even if you change the size
     * of the font.
     *
     ...
     * is useful for determining the height of the font below the baseline.
     *
     * @webref typography:metrics
     ...
     * Fonts must be created for Processing with <b>createFont()</b> or loaded
     * with
     * <b>loadFont()</b> before they can be used. The font set through
     * <b>textFont()</b> will be used in all subsequent calls to the
     * <b>text()</b>
     * function. If no <b>size</b> parameter is specified, the font size
     * defaults to the original size (the size in which it was created with the
     * "Create Font..." tool) overriding any previous calls to <b>textFont()</b>
     * or
     * <b>textSize()</b>.<br />
     * <br />
     * When fonts are rendered as an image texture (as is the case with the P2D
     * and P3D renderers as well as with <b>loadFont()</b> and vlw files), you
     * should create fonts at the sizes that will be used most commonly. Using
     ...
     * @see UIFrame#createFont(String, float, boolean)
     * @see UIFrame#loadFont(String)
     * @see PFont
     * @see PGraphics#text(String, float, float)
     * @see PGraphics#textSize(float)
     ...
     * will be used in all subsequent calls to the <b>text()</b> function. Note,
     * however, that the leading is reset by <b>textSize()</b>. For example, if
     * the leading is set to 20 with <b>textLeading(20)</b>, then if
     * <b>textSize(48)</b> is run at a later point, the leading will be reset to
     * the default for the text size of 48.
     *
     * @webref typography:attributes
    ...
    /**
     * Sets the way text draws to the screen, either as texture maps or as
     * vector geometry. The default <b>textMode(MODEL)</b>, uses textures to
     * render the fonts. The <b>textMode(SHAPE)</b> mode draws text using the
     * glyph outlines of individual characters rather than as textures. This
     * mode is only supported with the <b>PDF</b> and <b>P3D</b> renderer
     * settings. With the <b>PDF</b>
     * renderer, you must call <b>textMode(SHAPE)</b> before any other drawing
     * occurs. If the outlines are not available, then <b>textMode(SHAPE)</b>
     * will be ignored and <b>textMode(MODEL)</b> will be used instead.<br />
     * <br />
     ...
     *
     * @webref typography:attributes
     * @webBrief Calculates and returns the width of any character or text
     * string
     ...
     * <b>textFont()</b> function and a default size will be used unless a font
     * is set with <b>textSize()</b>. Change the color of the text with the
     ...
     * display within and may only be used with string data. When these
     * parameters are specified, they are interpreted based on the current
     * <b>rectMode()</b>
     * setting. Text that does not fit completely within the rectangle specified
     * will not be drawn to the screen.<br />
     * <br />
     * Note that Processing now lets you call <b>text()</b> without first
     * specifying a PFont with <b>textFont()</b>. In that case, a generic
     * sans-serif font will be used instead. (See the third example above.)
     *
     * @webref typography:loading & displaying
     ...
     * their own nf() command, or if they want the long, ugly version of float,
     ...
     * settings and transformations, while <b>pop()</b> restores these settings
     * @see PGraphics#pop()
     */
    public void push() {
        if (recorder != null) {
     ...
     * conjunction with the other transformation functions and may be embedded
     ...
     * This function can be further controlled by using <b>pushMatrix()</b> and
     * <b>popMatrix()</b>.
     *
     * @webref transform
     * @webBrief Specifies an amount to displace objects within the display
     * window
     ...
     *
     * @webref transform
     * @webBrief Rotates a shape the amount specified by the <b>angle</b>
     * parameter
     ...
     * <br />
     * Transformations apply to everything that happens after and subsequent
     * calls to the function multiply the effect. For example, calling
     * <b>scale(2.0)</b>
     * and then <b>scale(1.5)</b> is the same as <b>scale(3.0)</b>. If
     * <b>scale()</b> is called within <b>draw()</b>, the transformation is
     * reset when the loop begins again. Using this function with the <b>z</b>
     * parameter requires using P3D as a parameter for <b>size()</b>, as shown
     * in the third example above. This function can be further controlled with
     * <b>pushMatrix()</b> and <b>popMatrix()</b>.
     *
     * @webref transform
     ...
     * function in OpenGL is <b>glLoadIdentity()</b>.
     *
     * @webref transform
     ...
     *
     * @webref transform
     * @webBrief Prints the current matrix to the Console (the text window at
     ...
     * @webBrief The <b>beginCamera()</b> and <b>endCamera()</b> functions
     * enable advanced customization of the camera space
     ...
     *
     * @webref lights_camera:camera
     * @webBrief Prints the current camera matrix to the Console (the text
     * window at the bottom of Processing)
     * @see PGraphics#camera(float, float, float, float, float, float, float,
     * float, float)
     */
     ...
     * parameters are given, the default is used: <b>ortho(-width/2, width/2,
     * -height/2, height/2)</b>.
     *
     * @webref lights_camera:camera
     ...
     * values are: <b>perspective(PI/3.0, width/height, cameraZ/10.0,
     * cameraZ*10.0)</b> where cameraZ is <b>((height/2.0) /
     * tan(PI*60.0/360.0))</b>
     *
     * @webref lights_camera:camera
     ...
     * inside the clipping planes is rendered and visible; anything outside
     * those planes is not visible.<br />
     * <br />
     * Setting the frustum has the effect of changing the <em>perspective</em>
     * with which the scene is rendered. This can be achieved more simply in
     * many cases by using <strong>perspective()</strong>.<br />
     * <br />
     * Note that the near value must be greater than zero (as the point of the
     * frustum "pyramid" cannot converge "behind" the viewer). Similarly, the
     * far value must be greater than the near value (as the "far" plane of the
     * frustum must be "farther away" from the viewer than the near
     * plane).<br />
     * <br />
     ...
     * @webBrief Takes a three-dimensional X, Y, Z position and returns the X
     * value for where it will appear on a (two-dimensional) screen
     ...
     *
     * @webref lights_camera:coordinates
     * @webBrief Takes a three-dimensional X, Y, Z position and returns the Y
     * value for where it will appear on a (two-dimensional) screen
     * @param x 3D x-coordinate to be mapped
     * @param y 3D y-coordinate to be mapped
     ...
     *
     * @webref lights_camera:coordinates
     * @webBrief Takes a three-dimensional X, Y, Z position and returns the Z
     * value for where it will appear on a (two-dimensional) screen
     * @param x 3D x-coordinate to be mapped
     * @param y 3D y-coordinate to be mapped
     * @param z 3D z-coordinate to be mapped
     ...
     * <b>popMatrix()</b> is called, those transformations no longer apply, but
     * the
     * <b>(x, y, z)</b> coordinate returned by the model functions is used to
     * place another box in the same location.
     *
     * @webref lights_camera:coordinates
     ...
     * <b>textAlign()</b>, <b>textFont()</b>, <b>textMode()</b>,
     * <b>textSize()</b>, <b>textLeading()</b>,
     * <b>emissive()</b>, <b>specular()</b>, <b>shininess()</b>,
     * <b>ambient()</b>
     *
     * @webref structure
     * @webBrief Saves the current style settings and <b>popStyle()</b> restores
     * the prior settings
     ...
     * around shapes. All widths are set in units of pixels. <br />
     * <br />
     * Using point() with strokeWeight(1) or smaller may draw nothing to the
     * screen, depending on the graphics settings of the computer. Workarounds
     * include setting the pixel using <b>set()</s> or drawing the point using
     ...
     *
     * @webref shape:attributes
     * @webBrief Sets the width of the stroke used for lines, points, and the
     ...
     * <br />
     * To make <b>point()</b> appear square, use <b>strokeCap(PROJECT)</b>.
     * Using
     ...
     * @see PGraphics#strokeWeight(float)
     * @see PGraphics#strokeJoin(int)
     * @see UIFrame#size(int, int, String, String)
     */
     ...
     * the range from 0 to 255. <br />
     * <br />
     * When using hexadecimal notation to specify a color, use "<b>#</b>" or
     * "<b>0x</b>" before the values (e.g., <b>#CCFFAA</b> or
     * <b>0xFFCCFFAA</b>). The <b>#</b> syntax uses six digits to specify a
     * color (just as colors are typically specified in HTML and CSS). When
     * using the hexadecimal notation starting with "<b>0x</b>", the hexadecimal
     * value must be specified with eight characters; the first two characters
     * define the alpha component, and the remainder define the red, green, and
     * blue components. <br />
     * <br />
     ...
     * <b>hint(ENABLE_STROKE_PURE)</b> to improve drawing quality (at the
     * expense of performance). See the hint() documentation for more details.
     *
     * @webref color:setting
     ...
     *
     * @webref image:loading & displaying
     * @webBrief Removes the current fill value for displaying images and
     * reverts to displaying images with their original hues
     * @usage web_application
     ...
     * specified colors or made transparent by including an alpha value.<br />
     * <br />
     * To apply transparency to an image without affecting its color, use white
     * as the tint color and specify an alpha value. For instance, <b>tint(255,
     * 128)</b> will make an image 50% transparent (assuming the default alpha
     * range of 0-255, which can be changed with <b>colorMode()</b>). <br />
     * <br />
     * When using hexadecimal notation to specify a color, use "<b>#</b>" or
     * "<b>0x</b>" before the values (e.g., <b>#CCFFAA</b> or
     * <b>0xFFCCFFAA</b>). The <b>#</b> syntax uses six digits to specify a
    ...
    public void fill(Color x) {
        fill(x.getRed(), x.getGreen(), x.getBlue(), x.getAlpha());
    }

    public void stroke(Color x) {
        stroke(x.getRed(), x.getGreen(), x.getBlue(), x.getAlpha());
    }
     ...
     *
     * @webref lights_camera:material properties
     * @webBrief Sets the specular color of the materials used for shapes drawn
     * to the screen, which sets the color of highlights
     * @usage web_application
     ...
     * direction, the rays of light have bounced around so much that objects are
     ...
     * <b>draw()</b> to remain persistent in a looping program. Placing them in
     * the <b>setup()</b> of a looping program will cause them to only have an
     * effect the first time through the loop. The <b>v1</b>, <b>v2</b>, and
     * <b>v3</b> parameters are interpreted as either RGB or HSB values,
     ...
     * effect the first time through the loop. The <b>v1</b>, <b>v2</b>, and
     * <b>v3</b> parameters are interpreted as either RGB or HSB values,
     * depending on the current color mode. The
     * <b>x</b>, <b>y</b>, and <b>z</b> parameters set the position of the
     * light.
     *
     * @webref lights_camera:lights
    ...
    /**
     *
     * Adds a spotlight. Lights need to be included in the <b>draw()</b> to
     * remain persistent in a looping program. Placing them in the
     * <b>setup()</b> of a looping program will cause them to only have an
     * effect the first time through the loop. The <b>v1</b>, <b>v2</b>, and
     * <b>v3</b> parameters are interpreted as either RGB or HSB values,
     * depending on the current color mode. The
     * <b>x</b>, <b>y</b>, and <b>z</b> parameters specify the position of the
     * light and <b>nx</b>, <b>ny</b>, <b>nz</b> specify the direction of light.
     * The
     ...
     * Like <b>fill()</b>, it affects only the elements which are created after
     * it in the code. The default value is <b>lightFalloff(1.0, 0.0, 0.0)</b>,
     * and the parameters are used to calculate the falloff with the following
     * equation:<br />
     * <br />
     * d = distance from light position to vertex position<br />
     * falloff = 1 / (CONSTANT + d * LINEAR + (d*d) * QUADRATIC)<br />
     * <br />
     * Thinking about an ambient light with a falloff can be tricky. If you want
     * a region of your scene to be ambient lit with one color and another
     * region to be ambient lit with another color, you could use an ambient
     * light with location and falloff. You can think of it as a point light
     * that doesn't care which direction a surface is facing.
     *
     * @webref lights_camera:lights
     * @webBrief Sets the falloff rates for point lights, spotlights, and
     * ambient lights
     * @usage web_application
     ...
     * of the Processing window. The default background is light gray. This
     * function is typically used within <b>draw()</b> to clear the display
     * window at the beginning of each frame, but it can be used inside
     * <b>setup()</b> to set the background on the first frame of animation or
     * if the background need only be set once. <br />
     * <br />
     ...
     * used with <b>background()</b> will ignore the current <b>tint()</b>
     * setting. To resize an image to the size of the sketch window, use
     * image.resize(width, height). <br />
     * <br />
     * It is not possible to use the transparency <b>alpha</b> parameter with
     * background colors on the main drawing surface. It can only be used along
     * with a <b>PGraphics</b> object and <b>createGraphics()</b>.
     *
     * <h3>Advanced</h3>
     * <p>
     ...
     * function. Unlike the main graphics context (the display window), pixels
     * in additional graphics areas created with <b>createGraphics()</b>
     * can be entirely or partially transparent. This function clears everything
     * in a <b>PGraphics</b> object to make all the pixels 100% transparent.
     *
     * @webref color:setting
     ...
     * <b>colorMode()</b> function is used to change the numerical range used
     * for specifying colors and to switch color systems. For example, calling
     * <b>colorMode(RGB, 1.0)</b> will specify that values are specified between
     * 0 and 1. The limits for defining colors are altered by setting the
     * parameters
     * <b>max</b>, <b>max1</b>, <b>max2</b>, <b>max3</b>, and <b>maxA</b>.
     * <br />
     * <br />
     * After changing the range of values for colors with code like
     * <b>colorMode(HSB, 360, 100, 100)</b>, those ranges remain in use until
     * they are explicitly changed again. For example, after running
     * <b>colorMode(HSB, 360, 100, 100)</b> and then changing back to
     * <b>colorMode(RGB)</b>, the range for R will be 0 to 360 and the range for
     * G and B will be 0 to 100. To avoid this, be explicit about the ranges
     * when changing the color mode. For instance, instead of
     * <b>colorMode(RGB)</b>, write <b>colorMode(RGB, 255, 255, 255)</b>.
     *
     * @webref color:setting
     ...
     * <b>colorMode()</b>. The value is always returned as a float, so be
     * careful not to assign it to an int value.<br />
     * <br />
     ...
     * 255)</b>, you can achieve the same results as <b>red()</b> but with
     * greater speed by using the right shift operator (<b>>></b>) with a bit
     * mask. For example, the following two lines of code are equivalent means
     * of getting the red value of the color value <b>c</b>:<br />
     * <br />
     *
     * <pre>
     ...
     * careful not to assign it to an int value.<br />
     * <br />
     * The <b>green()</b> function is easy to use and understand, but it is
     * slower than a technique called bit shifting. When working in
     * <b>colorMode(RGB, 255)</b>, you can achieve the same results as
     * <b>green()</b> but with greater speed by using the right shift operator
     * (<b>>></b>) with a bit mask. For example, the following two lines of code
     * are equivalent means of getting the green value of the color value
     * <b>c</b>:<br />
     * <br />
     *
     * <pre>
     ...
     * careful not to assign it to an int value.<br />
     * <br />
     * The <b>blue()</b> function is easy to use and understand, but it is
     * slower than a technique called bit masking. When working in
     * <b>colorMode(RGB, 255)</b>, you can achieve the same results as
     * <b>blue()</b> but with greater speed by using a bit mask to remove the
     * other color components. For example, the following two lines of code are
     * equivalent means of getting the blue value of the color value
     * <b>c</b>:<br />
     * <br />
     *
     * <pre>
     ...
     * @see UIFrame#set(int, int, int)
     * @see UIFrame#pixels
     * @see UIFrame#copy(PImage, int, int, int, int, int, int, int, int)
     */
     ...
     * <br />
     * THRESHOLD<br />
     * Converts the image to black and white pixels depending on if they are
     * above or below the threshold defined by the level parameter. The
     ...
     * results are most noticeable in the lower ranges.<br />
     * <br />
     * BLUR<br />
     * Executes a Gaussian blur with the level parameter specifying the extent
     * of the blurring. If no parameter is used, the blur is equivalent to
     * Gaussian blur of radius 1. Larger values increase the blur.<br />
     * <br />
     * ERODE<br />
     ...
     *
     * <h3>Advanced</h3> Method to apply a variety of basic filters to this
     * image.
     * <P>
     * <UL>
     ...
     * 0)</b><br />
     * <br />
     * DARKEST - only the darkest colour succeeds: <b>C = min(A*factor,
     * B)</b><br />
     * <br />
