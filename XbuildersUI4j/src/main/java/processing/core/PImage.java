 ...
 * @see UIFrame#loadImage(String)
 * @see UIFrame#imageMode(int)
 * @see UIFrame#createImage(int, int, int)
 */
...
public class PImage implements PConstants, Cloneable {
     ...
     * @throws IOException
     */
    public PImage(File file) throws IOException {
        ShimAWT.fromNativeImage(ImageIO.read(file), this);
    }
     ...
     * image. These values are of the color datatype. This array is the size of
     * the image, meaning if the image is 100 x 100 pixels, there will be 10,000
     * values and if the window is 200 x 300 pixels, there will be 60,000
     * values. <br />
     * <br />
     ...
     * <b>updatePixels()</b> method must be run to update the content of the
     * display window.
     *
     * @webref image:pixels
     ...
     * Processing can display <b>.gif</b>,
     ...
     * constructor, or re-init later when things change. Used by Capture and
    ...
    public PImage(int width, int height, int[] pixels,
            boolean requiresCheckAlpha, UIFrame parent) {
     ...
     *
     * <h3>Advanced</h3> Call this when you want to mess with the pixels[]
     * array.
     * <p/>
     ...
     * @webBrief Loads the pixel data for the image into its <b>pixels[]</b>
     * array
     * @usage web_application
     */
     ...
     *
     * <h3>Advanced</h3> Mark the pixels in this region as needing an update.
    ...
    public void updatePixels(int x, int y, int w, int h) {  // ignore
        ...
        if (!modified) {
            mx1 = UIFrame.max(0, x);
            mx2 = UIFrame.min(pixelWidth, x2);
            my1 = UIFrame.max(0, y);
            my2 = UIFrame.min(pixelHeight, y2);
            modified = true;
            ...
            if (x > mx2) {
                mx2 = UIFrame.min(pixelWidth, x);
            }
            if (y < my1) {
                my1 = UIFrame.max(0, y);
            }
            if (y > my2) {
                my2 = UIFrame.min(pixelHeight, y);
            }

            if (x2 < mx1) {
                mx1 = UIFrame.max(0, x2);
            }
            if (x2 > mx2) {
                mx2 = UIFrame.min(pixelWidth, x2);
            }
            if (y2 < my1) {
                my1 = UIFrame.max(0, y2);
            }
            if (y2 > my2) {
                my2 = UIFrame.min(pixelHeight, y2);
            }
     ...
     * change the height using the same proportion, use <b>resize(150,
     * 0)</b>.<br />
     * <br />
     * Even though a PGraphics is technically a <b>PImage</b>, it is not
     * possible to rescale the image data found in a <b>PGraphics</b>. (It's
     * simply not possible to do this consistently across renderers: technically
     * infeasible with P3D, or what would it even do with PDF?) If you want to
     * resize <b>PGraphics</b>
    ...
    public int get(int x, int y) {
        ...
        return switch (format) {
            case RGB ->
                pixels[y * pixelWidth + x] | 0xff000000;
            case ARGB ->
                pixels[y * pixelWidth + x];
            case ALPHA ->
                (pixels[y * pixelWidth + x] << 24) | 0xffffff;
            default ->
                0;
        };
    }
...
//          currLum = lumDown;  // removed, unused assignment
     ...
     * preferable, and the difference is insignificant for applications built
     * with Processing.</P>
     *
     * @webref color:creating & reading
     * @webBrief Blends two color values together based on the blending mode
     * given as the
     * <b>MODE</b> parameter
     * @usage web_application
     ...
     * BURN
     * @see PImage#blend(PImage, int, int, int, int, int, int, int, int, int)
     * @see UIFrame#color(float, float, float, float)
     */
    static public int blendColor(int c1, int c2, int mode) {  // ignore
        return switch (mode) {
            ...
            case SUBTRACT ->
                blend_sub_pin(c1, c2);
            case LIGHTEST ->
                blend_lightest(c1, c2);
            case DARKEST ->
                blend_darkest(c1, c2);
            case DIFFERENCE ->
                blend_difference(c1, c2);
            case EXCLUSION ->
                blend_exclusion(c1, c2);
            case MULTIPLY ->
                blend_multiply(c1, c2);
            case SCREEN ->
                blend_screen(c1, c2);
            case HARD_LIGHT ->
                blend_hard_light(c1, c2);
            case SOFT_LIGHT ->
                blend_soft_light(c1, c2);
            case OVERLAY ->
                blend_overlay(c1, c2);
            case DODGE ->
                blend_dodge(c1, c2);
...
int testFunction(int dst, int src) {
   ...
   * O = DS
    ...
    /**
     * Soft Light (peg top) O = (1 - D) * MULTIPLY(D, S) + D * SCREEN(D, S) O =
     ...
     * savePath() from UIFrame, or use saveFrame() instead. As of revision 0116,
     ...
     * <p>
     * Also, files must have an extension: we're no longer adding .tif to files
     * with no extension, because that can lead to confusing results, and the
     * behavior is inconsistent with the rest of the API.
     *
     * @webref pimage:method
    ...
    /**
     * Override this in subclasses to intercept save calls for other formats or
     * higher-performance implementations. When reaching this code, pixels must
     * be loaded and that path should be absolute.
     *
