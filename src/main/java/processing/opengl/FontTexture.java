...
class FontTexture implements PConstants {
  ...
  protected void initTexture(PGraphicsOpenGL pg, PFont font) {
    ...
    minSize = UIFrame.min(PGraphicsOpenGL.maxTextureSize,
                          UIFrame.max(PGL.MIN_FONT_TEX_SIZE, spow));
    maxSize = UIFrame.min(PGraphicsOpenGL.maxTextureSize,
                          UIFrame.max(PGL.MAX_FONT_TEX_SIZE, 2 * spow));
  ...
  public boolean addTexture(PGraphicsOpenGL pg) {
    ...
    if (-1 < lastTex && textures[lastTex].glHeight < maxSize) {
      ...
      h = UIFrame.min(2 * textures[lastTex].glHeight, maxSize);
      resize = true;
    } else {
      h = minSize;
      resize = false;
    ...
    if (textures == null) {
      ...
      UIFrame.arrayCopy(textures, tempTex, textures.length);
      tempTex[lastTex] = tex;
      textures = tempTex;

      PImage[] tempImg = new PImage[textures.length];
      UIFrame.arrayCopy(images, tempImg, images.length);
