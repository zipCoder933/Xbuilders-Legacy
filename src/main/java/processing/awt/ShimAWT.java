...
public class ShimAWT implements PConstants {
  ...
  */
  ...
  private ShimAWT() {
    ...
    // UIFrame to cache and the values to stick through subsequent runs.
...
//  }
  ...
  static public PImage loadImage(UIFrame sketch, String filename, Object... args) {
    ...
    */
    ...
    if (loadImageExtensions != null) {
      ...
      System.err.println("Could not load " + filename + ", " +
        ...
        "(" + UIFrame.join(loadImageExtensions, ", ") + ")");
    } else {
      System.err.println("Could not load " + filename);
    }
    return null;
   ...
   */
  static protected PImage loadImageIO(UIFrame sketch, String filename) {
    ...
    try {
      if (saveImageExtensions != null) {
        ...
        System.err.println("Could not save " + path + ", " +
                           "make sure it ends with a supported extension " +
                           "(" + UIFrame.join(saveImageExtensions, ", ") + ")");
      }
    } catch (IOException e) {
      e.printStackTrace();
    }
  ...
  static protected boolean saveImageIO(PImage image, String path, String... args) throws IOException {
    try {
      ...
      if (writer != null) {
        OutputStream output = UIFrame.createOutput(file);
        if (output == null) {
          return false;
        }
    ...
    EventQueue.invokeLater(new Runnable() {
      public void run() {
        boolean hide = (sketch != null) &&
          ...
          (UIFrame.platform == PConstants.WINDOWS);
  ...
  */
  ...
  static public void selectImpl(final String prompt,
    ...
    UIFrame.selectCallback(selectedFile, callbackMethod, callbackObject);
  }
  ...
  */
  ...
  static public void selectFolderImpl(final String prompt,
                                      ...
                                      final Frame parentFrame) {
    File selectedFile = null;
    if (UIFrame.platform == PConstants.MACOS && UIFrame.useNativeSelect) {
  ...
  static private void checkLookAndFeel() {
    if (!lookAndFeelCheck) {
      if (UIFrame.platform == PConstants.WINDOWS) {
  ...
  // TODO maybe call this with reflection from inside UIFrame?
