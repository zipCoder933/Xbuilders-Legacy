...
//  /**
//   * Begin parsing XML data passed in from a UIFrame. This code
...
//   * @throws IOException
//   */
//  public XML(UIFrame parent, String filename) throws IOException, ParserConfigurationException, SAXException {
...
//  }
  ...
  /**
   * Unlike the loadXML() method in UIFrame, this version works with files
 that are not in UTF-8 format.
  ...
  public XML(InputStream input, String options) throws IOException, ParserConfigurationException, SAXException {
    //this(UIFrame.createReader(input), options);  // won't handle non-UTF8
...
//    return write(UIFrame.createWriter(output));
//  }
  ...
  public boolean save(File file, String options) {
    PrintWriter writer = UIFrame.createWriter(file);
...
//    return kids;
  ...
  public XML getChild(String name) {
    ...
    if (name.indexOf('/') != -1) {
      return getChildRecursive(UIFrame.split(name, '/'), 0);
    }
  ...
  public XML[] getChildren(String name) {
    ...
    if (name.indexOf('/') != -1) {
      return getChildrenRecursive(UIFrame.split(name, '/'), 0);
    }
    // if it's a number, do an index instead
    ...
    return (XML[]) UIFrame.subset(matches, 0, matchCount);
  }
  ...
  protected XML[] getChildrenRecursive(String[] items, int offset) {
    ...
    for (int i = 0; i < matches.length; i++) {
      ...
      outgoing = (XML[]) UIFrame.concat(outgoing, kidMatches);
    }
    return outgoing;
  }
  ...
  protected XML appendChild(Node newNode) {
    ...
    if (children != null) {
      children = (XML[]) UIFrame.concat(children, new XML[] { newbie });
    }
    return newbie;
  }
...
//      children = (XML[]) UIFrame.subset(children, 0, index);
//    }
//
...
//  }
  ...
  public int getIntContent(int defaultValue) {
    return UIFrame.parseInt(node.getTextContent(), defaultValue);
  }
  ...
  public float getFloatContent(float defaultValue) {
    return UIFrame.parseFloat(node.getTextContent(), defaultValue);
  }
...
//      }
      ...
      String[] tempLines = UIFrame.split(tempWriter.toString(), sep);
//      UIFrame.println(tempLines);
...
//          UIFrame.println("removing first line");
          tempLines = UIFrame.subset(tempLines, 1);
        } else {
//          UIFrame.println("removing part of first line");
      ...
      String singleLine = UIFrame.join(UIFrame.trim(tempLines), "");
      if (indent == -1) {
        return singleLine;
      }
