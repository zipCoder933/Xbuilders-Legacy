...
public class Table {
  ...
  public Table(File file, String options) throws IOException {
    ...
    init();
    parse(UIFrame.createInput(file),
...
//        addRow(row);
  ...
  */
  ...
  static public String extensionOptions(boolean loading, String filename, String options) {
    String extension = UIFrame.checkExtension(filename);
    if (extension != null) {
...
//    boolean awfulCSV = false;
    ...
    String[] opts = null;
    if (options != null) {
      opts = UIFrame.trim(UIFrame.split(options, ','));
      for (String opt : opts) {
        if (opt.equals("tsv")) {
          extension = "tsv";
          ...
          // ignore option, this is only handled by UIFrame
  ...
  protected void parseBasic(BufferedReader reader,
    ...
    try {
      while ((line = reader.readLine()) != null) {
        ...
        if (row == 0 && header) {
          setColumnTitles(tsv ? UIFrame.split(line, '\t') : splitLineCSV(line, reader));
          header = false;
        } else {
          setRow(row, tsv ? UIFrame.split(line, '\t') : splitLineCSV(line, reader));
          row++;
        }
...
//          if (row % 1000 == 0) {
//            UIFrame.println(UIFrame.nfc(row));
//          }
          if (row == alloc) {
            alloc *= 2;
            setRowCount(alloc);
...
//      UIFrame.println("handle() called for: " + line);
      start = 0;
      pieceCount = 0;
      c = line.toCharArray();
...
//            System.err.println(line);
          ...
          UIFrame.arrayCopy(c, temp, c.length);
...
//      UIFrame.println("found " + targetField);
...
//    UIFrame.println("enclosing obj is " + enclosingObject);
...
//        UIFrame.println("no enclosing class");
      } else {
        con = target.getDeclaredConstructor(enclosingClass);
//        UIFrame.println("enclosed by " + enclosingClass.getName());
      }
...
//        System.out.println("skipping field " + name);
    ...
    try {
      for (TableRow row : rows()) {
        ...
        if (index == 0) {
          for (Field field : inuse) {
            if (!field.canAccess(item)) {
              // UIFrame.println("  changing field access");
        ...
        for (Field field : inuse) {
          String name = field.getName();
          // UIFrame.println("gonna set field " + name);
...
//        UIFrame.println("setting target field to public");
...
//        writer.print("&apos;");
  ...
  protected void saveODS(OutputStream os) throws IOException {
    ...
    */
    ...
    byte[] dummyBytes = UIFrame.join(lines, "\n").getBytes();
  ...
  protected void loadBinary(InputStream is) throws IOException {
    ...
    if (magic != 0x9007AB1E) {
      throw new IOException("Not a compatible binary table (magic was " + UIFrame.hex(magic) + ")");
    }
  ...
  public void insertColumn(int index, String title, int type) {
    ...
    if (columnTitles != null) {
      columnTitles = UIFrame.splice(columnTitles, title, index);
      columnIndices = null;
    }
    columnTypes = UIFrame.splice(columnTypes, type, index);

//    columnCategories = (HashMapBlows[])
//      UIFrame.splice(columnCategories, new HashMapBlows(), index);
 ...
 /**
  ...
  public void setColumnCount(int newCount) {
    ...
    if (oldCount != newCount) {
      columns = (Object[]) UIFrame.expand(columns, newCount);
      ...
      if (columnTitles != null) {
        columnTitles = UIFrame.expand(columnTitles, newCount);
      }
      columnTypes = UIFrame.expand(columnTypes, newCount);
      columnCategories = (HashMapBlows[])
        UIFrame.expand(columnCategories, newCount);
    }
  }
    ...
    switch (newType) {
      case INT: {
        ...
        for (int row = 0; row < rowCount; row++) {
          ...
          intData[row] = (s == null) ? missingInt : UIFrame.parseInt(s, missingInt);
        }
      ...
      case FLOAT: {
        ...
        for (int row = 0; row < rowCount; row++) {
          String s = getString(row, column);
          floatData[row] = (s == null) ? missingFloat : UIFrame.parseFloat(s, missingFloat);
        }
...
//          setColumnType(i, dictionary.getString(i, typeCol));
  ...
  public void setRowCount(int newCount) {
    if (newCount != rowCount) {
      if (newCount > 1000000) {
        System.out.print("Note: setting maximum row count to " + UIFrame.nfc(newCount));
      }
      long t = System.currentTimeMillis();
      for (int col = 0; col < columns.length; col++) {
        switch (columnTypes[col]) {
          case INT: columns[col] = UIFrame.expand((int[]) columns[col], newCount); break;
          case LONG: columns[col] = UIFrame.expand((long[]) columns[col], newCount); break;
          case FLOAT: columns[col] = UIFrame.expand((float[]) columns[col], newCount); break;
          case DOUBLE: columns[col] = UIFrame.expand((double[]) columns[col], newCount); break;
          case STRING: columns[col] = UIFrame.expand((String[]) columns[col], newCount); break;
          case CATEGORY: columns[col] = UIFrame.expand((int[]) columns[col], newCount); break;
        }
        if (newCount > 1000000) {
          try {
      ...
      if (newCount > 1000000) {
        ...
        System.out.println(" (resize took " + UIFrame.nfc(ms) + " ms)");
      }
    }
    rowCount = newCount;
  }
...
//          System.arraycopy(doubleData, dead+1, doubleTemp, dead, (rowCount - dead) + 1);
  ...
  protected void setRowCol(int row, int col, String piece) {
    ...
    case INT:
      int[] intData = (int[]) columns[col];
      intData[row] = UIFrame.parseInt(piece, missingInt);
      break;
    case LONG:
      long[] longData = (long[]) columns[col];
      try {
    ...
    case FLOAT:
      float[] floatData = (float[]) columns[col];
      floatData[row] = UIFrame.parseFloat(piece, missingFloat);
      break;
    case DOUBLE:
      double[] doubleData = (double[]) columns[col];
      try {
...
//          stringData[row] = (String) piece;
      ...
      case INT:
        ...
        if (piece == null) {
          ...
          intData[row] = UIFrame.parseInt(String.valueOf(piece), missingInt);
        }
        break;
      case LONG:
        long[] longData = (long[]) columns[col];
      ...
      case FLOAT:
        ...
        if (piece == null) {
          ...
          floatData[row] = UIFrame.parseFloat(String.valueOf(piece), missingFloat);
        }
        break;
      case DOUBLE:
        double[] doubleData = (double[]) columns[col];
  ...
  */
  ...
  public int getInt(int row, int column) {
    ...
    return (str == null || str.equals(missingString)) ?
      missingInt : UIFrame.parseInt(str, missingInt);
  }
  ...
  public float getFloat(int row, int column) {
    ...
    return UIFrame.parseFloat(str, missingFloat);
  }
...
//  }
  ...
  public int[] findRowIndices(String value, int column) {
    ...
    return UIFrame.subset(outgoing, 0, count);
  }
  ...
  public int matchRowIndex(String regexp, int column) {
    ...
    if (columnTypes[column] == STRING) {
      ...
      for (int row = 0; row < rowCount; row++) {
        if (stringData[row] != null &&
            UIFrame.match(stringData[row], regexp) != null) {
          return row;
        }
      }
    } else {  // less efficient, includes conversion as necessary
   ...
   */
  public void trim() {
    columnTitles = UIFrame.trim(columnTitles);
    for (int col = 0; col < getColumnCount(); col++) {
      trim(col);
    }
  ...
  */
  ...
  public void trim(int column) {
    if (columnTypes[column] == STRING) {
      ...
      for (int row = 0; row < rowCount; row++) {
        if (stringData[row] != null) {
          stringData[row] = UIFrame.trim(stringData[row]);
        }
      }
    }
  }
...
//  }
  ...
  protected void convertBasic(BufferedReader reader, boolean tsv,
    ...
    int row = 0;
    while ((line = reader.readLine()) != null) {
      convertRow(output, tsv ? UIFrame.split(line, '\t') : splitLineCSV(line, reader));
      row++;
...
//    }
    ...
    for (HashMapBlows hmb : columnCategories) {
      if (hmb == null) {
        ...
        hmb.write(output);
        hmb.writeln(UIFrame.createWriter(new File(columnTitles[col] + ".categories")));
  ...
  protected void convertRow(DataOutputStream output, String[] pieces) throws IOException {
    ...
    for (int col = 0; col < pieces.length; col++) {
      ...
      case STRING:
        ...
        break;
      case INT:
        output.writeInt(UIFrame.parseInt(pieces[col], missingInt));
        break;
      case LONG:
        try {
        ...
        break;
      case FLOAT:
        output.writeFloat(UIFrame.parseFloat(pieces[col], missingFloat));
        break;
      case DOUBLE:
        try {
