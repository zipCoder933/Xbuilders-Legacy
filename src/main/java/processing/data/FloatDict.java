...
public class FloatDict {
  ...
  public FloatDict(BufferedReader reader) {
    String[] lines = UIFrame.loadStrings(reader);
    keys = new String[lines.length];
    values = new float[lines.length];

    for (int i = 0; i < lines.length; i++) {
      String[] pieces = UIFrame.split(lines[i], '\t');
      if (pieces.length == 2) {
        keys[count] = pieces[0];
        values[count] = UIFrame.parseFloat(pieces[1]);
  ...
  public void resize(int length) {
    ...
    UIFrame.arrayCopy(keys, newKeys, length);
    UIFrame.arrayCopy(values, newValues, length);
    keys = newKeys;
    values = newValues;
    count = length;
    resetIndices();
  ...
  protected void crop() {
    if (count != keys.length) {
      keys = UIFrame.subset(keys, 0, count);
      values = UIFrame.subset(values, 0, count);
    }
  }
  ...
  protected void create(String what, float much) {
    if (count == keys.length) {
      keys = UIFrame.expand(keys);
      values = UIFrame.expand(values);
    }
