...
public class PShapeSVG extends PShape {
  ...
  protected PShapeSVG(PShapeSVG parent, XML properties, boolean parseKids) {
    ...
    if (properties.getName().equals("svg")) {
      ...
      if (viewBoxStr != null) {
        float[] viewBox = UIFrame.parseFloat(UIFrame.splitTokens(viewBoxStr));
      ...
      svgWidth = width;
      svgHeight = height;
      svgSizeXY = UIFrame.sqrt((svgWidth*svgWidth + svgHeight*svgHeight)/2.0f);
    }
    ...
    if (name != null) {
      while (true) {
        String[] m = UIFrame.match(name, "_x([A-Za-z0-9]{2})_");
        if (m == null) break;
        char repair = (char) UIFrame.unhex(m[1]);
...
//      fillGradientPaint = parent.fillGradientPaint;
  ...
  protected void parseChildren(XML graphics) {
    ...
    children = (PShape[]) UIFrame.subset(children, 0, childCount);
  }
...
//      String[] pointsBuffer = UIFrame.splitTokens(pointsAttr);
//      vertexCount = pointsBuffer.length;
//      vertices = new float[vertexCount][2];
//      for (int i = 0; i < vertexCount; i++) {
//        String pb[] = UIFrame.splitTokens(pointsBuffer[i], ", \t\r\n");
...
//      }
  ...
  protected void parsePath() {
    ...
    if (pathData == null || UIFrame.trim(pathData).length() == 0) {
      return;
    }
    ...
    String[] pathTokens =
      UIFrame.splitTokens(pathBuffer.toString(), "|" + WHITESPACE);
    ...
    while (i < pathTokens.length) {
      ...
      case 'M':  // M - move to (absolute)
        cx = UIFrame.parseFloat(pathTokens[i + 1]);
        cy = UIFrame.parseFloat(pathTokens[i + 2]);
        movetoX = cx;
        movetoY = cy;
      ...
      case 'm':  // m - move to (relative)
        cx = cx + UIFrame.parseFloat(pathTokens[i + 1]);
        cy = cy + UIFrame.parseFloat(pathTokens[i + 2]);
        movetoX = cx;
        movetoY = cy;
        parsePathMoveto(cx, cy);
        implicitCommand = 'l';
      ...
      case 'V':
        cy = UIFrame.parseFloat(pathTokens[i + 1]);
        parsePathLineto(cx, cy);
        i += 2;
        break;

      case 'v':
        cy = cy + UIFrame.parseFloat(pathTokens[i + 1]);
        parsePathLineto(cx, cy);
        i += 2;
        break;
        ...
        // C - curve to (absolute)
      case 'C': {
        float ctrlX1 = UIFrame.parseFloat(pathTokens[i + 1]);
        float ctrlY1 = UIFrame.parseFloat(pathTokens[i + 2]);
        float ctrlX2 = UIFrame.parseFloat(pathTokens[i + 3]);
        float ctrlY2 = UIFrame.parseFloat(pathTokens[i + 4]);
        float endX = UIFrame.parseFloat(pathTokens[i + 5]);
        float endY = UIFrame.parseFloat(pathTokens[i + 6]);
      ...
      break;
        ...
        // c - curve to (relative)
      case 'c': {
        float ctrlX1 = cx + UIFrame.parseFloat(pathTokens[i + 1]);
        float ctrlY1 = cy + UIFrame.parseFloat(pathTokens[i + 2]);
        float ctrlX2 = cx + UIFrame.parseFloat(pathTokens[i + 3]);
        float ctrlY2 = cy + UIFrame.parseFloat(pathTokens[i + 4]);
        float endX = cx + UIFrame.parseFloat(pathTokens[i + 5]);
        float endY = cy + UIFrame.parseFloat(pathTokens[i + 6]);
        parsePathCurveto(ctrlX1, ctrlY1, ctrlX2, ctrlY2, endX, endY);
        cx = endX;
        cy = endY;
        i += 7;
      ...
      case 'S': {
        ...
        float ctrlX2 = UIFrame.parseFloat(pathTokens[i + 1]);
        float ctrlY2 = UIFrame.parseFloat(pathTokens[i + 2]);
        float endX = UIFrame.parseFloat(pathTokens[i + 3]);
        float endY = UIFrame.parseFloat(pathTokens[i + 4]);
      ...
      case 's': {
        ...
        float ctrlX2 = cx + UIFrame.parseFloat(pathTokens[i + 1]);
        float ctrlY2 = cy + UIFrame.parseFloat(pathTokens[i + 2]);
        float endX = cx + UIFrame.parseFloat(pathTokens[i + 3]);
        float endY = cy + UIFrame.parseFloat(pathTokens[i + 4]);
        parsePathCurveto(ctrlX, ctrlY, ctrlX2, ctrlY2, endX, endY);
        cx = endX;
        cy = endY;
        i += 5;
      ...
      // becomes the final (x,y) coordinate pair used in the polybézier.
      case 'Q': {
        ctrlX = UIFrame.parseFloat(pathTokens[i + 1]);
        ctrlY = UIFrame.parseFloat(pathTokens[i + 2]);
        float endX = UIFrame.parseFloat(pathTokens[i + 3]);
        float endY = UIFrame.parseFloat(pathTokens[i + 4]);
      ...
      case 'q': {
        ctrlX = cx + UIFrame.parseFloat(pathTokens[i + 1]);
        ctrlY = cy + UIFrame.parseFloat(pathTokens[i + 2]);
        float endX = cx + UIFrame.parseFloat(pathTokens[i + 3]);
        float endY = cy + UIFrame.parseFloat(pathTokens[i + 4]);
        //parsePathQuadto(cx, cy, ctrlX, ctrlY, endX, endY);
        parsePathQuadto(ctrlX, ctrlY, endX, endY);
      ...
      case 'T': {
        ...
        float endX = UIFrame.parseFloat(pathTokens[i + 1]);
        float endY = UIFrame.parseFloat(pathTokens[i + 2]);
        //parsePathQuadto(cx, cy, ctrlX, ctrlY, endX, endY);
        parsePathQuadto(ctrlX, ctrlY, endX, endY);
        cx = endX;
        cy = endY;
      ...
      case 't': {
        ...
        float endX = cx + UIFrame.parseFloat(pathTokens[i + 1]);
        float endY = cy + UIFrame.parseFloat(pathTokens[i + 2]);
        //parsePathQuadto(cx, cy, ctrlX, ctrlY, endX, endY);
        parsePathQuadto(ctrlX, ctrlY, endX, endY);
        cx = endX;
        cy = endY;
      ...
      case 'A': {
        float rx = UIFrame.parseFloat(pathTokens[i + 1]);
        float ry = UIFrame.parseFloat(pathTokens[i + 2]);
        float angle = UIFrame.parseFloat(pathTokens[i + 3]);
        boolean fa = UIFrame.parseFloat(pathTokens[i + 4]) != 0;
        boolean fs = UIFrame.parseFloat(pathTokens[i + 5]) != 0;
        float endX = UIFrame.parseFloat(pathTokens[i + 6]);
        float endY = UIFrame.parseFloat(pathTokens[i + 7]);
      ...
      case 'a': {
        ...
        boolean fa = UIFrame.parseFloat(pathTokens[i + 4]) != 0;
        boolean fs = UIFrame.parseFloat(pathTokens[i + 5]) != 0;
        float endX = cx + UIFrame.parseFloat(pathTokens[i + 6]);
        float endY = cy + UIFrame.parseFloat(pathTokens[i + 7]);
        parsePathArcto(cx, cy, rx, ry, angle, fa, fs, endX, endY);
        cx = endX;
        cy = endY;
        i += 8;
      ...
      default:
        String parsed =
          UIFrame.join(UIFrame.subset(pathTokens, 0, i), ",");
        String unparsed =
          UIFrame.join(UIFrame.subset(pathTokens, i), ",");
...
//          //vertices = (float[][]) UIFrame.expand(vertices);
...
//      }
  ...
  private void parsePathCode(int what) {
    if (vertexCodeCount == vertexCodes.length) {
      vertexCodes = UIFrame.expand(vertexCodes);
    }
...
//    parsePathCode(BEZIER_VERTEX);
  ...
  private void parsePathArcto(float x1,    float y1,
    ...
    rx = UIFrame.abs(rx);  ry = UIFrame.abs(ry);

    float phi = UIFrame.radians(((angle % 360) + 360) % 360);
    float cosPhi = UIFrame.cos(phi),  sinPhi = UIFrame.sin(phi);
    ...
    float cxr, cyr;
    {
      ...
      if (A > 1) {
        ...
        float sqrtA = UIFrame.sqrt(A);
        rx *= sqrtA;  cxr = 0;
        ry *= sqrtA;  cyr = 0;
      } else {
        float k = ((fa == fs) ? -1f : 1f) *
          UIFrame.sqrt((rx*rx * ry*ry) / ((rx*rx * y1r*y1r) + (ry*ry * x1r*x1r)) - 1f);
        cxr =  k * rx * y1r / ry;
        cyr = -k * ry * x1r / rx;
      }
    }
    ...
    float phi1, phiDelta;
    {
      ...
      phi1 = UIFrame.atan2(sy, sx);
      phiDelta = (((UIFrame.atan2(ty, tx) - phi1) % TWO_PI) + TWO_PI) % TWO_PI;
    ...
    int segmentCount = UIFrame.ceil(UIFrame.abs(phiDelta) / TWO_PI * 4);

    float inc = phiDelta / segmentCount;
    float a = UIFrame.sin(inc) *
      (UIFrame.sqrt(4 + 3 * UIFrame.sq(UIFrame.tan(inc / 2))) - 1) / 3;

    float sinPhi1 = UIFrame.sin(phi1),  cosPhi1 = UIFrame.cos(phi1);
    ...
    for (int i = 0; i < segmentCount; i++) {
      ...
      float sinEta = UIFrame.sin(eta),  cosEta = UIFrame.cos(eta);
  ...
  static protected PMatrix2D parseSingleTransform(String matrixStr) {
    //String[] pieces = UIFrame.match(matrixStr, "^\\s*(\\w+)\\((.*)\\)\\s*$");
    String[] pieces = UIFrame.match(matrixStr, "[,\\s]*(\\w+)\\((.*)\\)");
    if (pieces == null) {
      System.err.println("Could not parse transform " + matrixStr);
      return null;
    }
    float[] m = UIFrame.parseFloat(UIFrame.splitTokens(pieces[2], ", "));
    if (pieces[1].equals("matrix")) {
      ...
      return new PMatrix2D(1, 0, 1,  UIFrame.tan(m[0]), 0, 0);

    } else if (pieces[1].equals("skewY")) {
      return new PMatrix2D(1, 0, 1,  0, UIFrame.tan(m[0]), 0);
    }
    return null;
  }
  ...
  protected void parseColors(XML properties) {
    ...
    if (properties.hasAttribute("style")) {
      ...
      String[] styleTokens = UIFrame.splitTokens(styleText, ";");

      //PApplet.println(styleTokens);
      for (int i = 0; i < styleTokens.length; i++) {
        String[] tokens = UIFrame.splitTokens(styleTokens[i], ":");
        //PApplet.println(tokens);

        tokens[0] = UIFrame.trim(tokens[0]);
  ...
  void setOpacity(String opacityText) {
    opacity = UIFrame.parseFloat(opacityText);
  ...
  void setStrokeOpacity(String opacityText) {
    strokeOpacity = UIFrame.parseFloat(opacityText);
  ...
  void setFillOpacity(String opacityText) {
    fillOpacity = UIFrame.parseFloat(opacityText);
...
//      strokeGradientPaint = paint;
  ...
  static protected int parseSimpleColor(String colorText) {
    ...
    if (colorNames.hasKey(colorText)) {
      ...
      //System.out.println("hex for fill is " + UIFrame.hex(fillColor));
  ...
  */
  ...
  static protected int parseRGB(String what) {
    ...
    String[] values = UIFrame.splitTokens(sub, ", ");
    int rgbValue = 0;
    if (values.length == 3) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        if (values[i].endsWith("%")) {
          rgbValue |= (int)(UIFrame.constrain(255*parseFloatOrPercent(values[i]), 0, 255));
        } else {
          rgbValue |= UIFrame.constrain(UIFrame.parseInt(values[i]), 0, 255);
        }
      }
...
//    if (style == null) return table;
  ...
  static protected float parseUnitSize(String text, float relativeTo) {
    ...
    if (text.endsWith("pt")) {
      return UIFrame.parseFloat(text.substring(0, len)) * 1.25f;
    } else if (text.endsWith("pc")) {
      return UIFrame.parseFloat(text.substring(0, len)) * 15;
    } else if (text.endsWith("mm")) {
      return UIFrame.parseFloat(text.substring(0, len)) * 3.543307f;
    } else if (text.endsWith("cm")) {
      return UIFrame.parseFloat(text.substring(0, len)) * 35.43307f;
    } else if (text.endsWith("in")) {
      return UIFrame.parseFloat(text.substring(0, len)) * 90;
    } else if (text.endsWith("px")) {
      return UIFrame.parseFloat(text.substring(0, len));
    } else if (text.endsWith("%")) {
      return relativeTo * parseFloatOrPercent(text);
    } else {
      return UIFrame.parseFloat(text);
    }
  }
    ...
    public Gradient(PShapeSVG parent, XML properties) {
      ...
      for (int i = 0; i < elements.length; i++) {
        ...
        if (name.equals("stop")) {
          ...
          int tupacity = UIFrame.constrain(
                          (int)(UIFrame.parseFloat(opacityStr) * 255), 0, 255);
          color[count] = (tupacity << 24) | parseSimpleColor(colorStr);
          count++;
        }
      }
      offset = UIFrame.subset(offset, 0, count);
      color = UIFrame.subset(color, 0, count);
    }
  }
    ...
    if (properties.hasAttribute("style")) {
      ...
      for (int i = 0; i < styleTokens.length; i++) {
        ...
        if (tokens[0].equals("font-style")) {
          // UIFrame.println("font-style: " + tokens[1]);
          if (tokens[1].contains("italic")) {
            italic = ITALIC;
          }
        } else if (tokens[0].equals("font-variant")) {
          // UIFrame.println("font-variant: " + tokens[1]);
          // setFillOpacity(tokens[1]);

        } else if (tokens[0].equals("font-weight")) {
          // UIFrame.println("font-weight: " + tokens[1]);

          if (tokens[1].contains("bold")) {
            weight = BOLD;
            // UIFrame.println("Bold weight ! ");
          }
