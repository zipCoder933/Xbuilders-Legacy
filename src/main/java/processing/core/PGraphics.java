 ...
 * <h2>Subclassing and initializing PGraphics objects</h2> Starting in release
 ...
 the hosting UIFrame to specify its attributes.
 <ul>
 * <li>setParent(UIFrame) - is called to specify the parent UIFrame.
 ...
 * <h2>Know your rights: public vs. private methods</h2> Methods that are
 ...
 accessible from UIFrame. That is, we don't want sketches calling
...
//  int matrixStackDepth;
  ...
  public void setParent(UIFrame parent) {  // ignore
    this.parent = parent;
...
//  }
 ...
 override setSize() in UIFrame/Applet/Component, which will 1) not call
...
//    }
  ...
  protected void reapplySettings() {
    ...
    if (fill) {
//      UIFrame.println("  fill " + UIFrame.hex(fillColor));
      fill(fillColor);
    } else {
      noFill();
    }
 ...
 */
  ...
  protected void curveVertexCheck(int shape) {
    ...
    if (curveVertexCount == curveVertices.length) {
      // Can't use UIFrame.expand() cuz it doesn't do the copy properly
...
//      if (currentY > y2) break;  // past the box
    ...
    //int lineFitCount = 1 + UIFrame.floor((boxHeight - textAscent()) / textLeading);
    ...
    int lineFitCount = 1 + UIFrame.floor((boxHeight - topAndBottom) / textLeading);
...
//    return y;
  ...
  protected void textSentenceBreak(int start, int stop) {
    if (textBreakCount == textBreakStart.length) {
      textBreakStart = UIFrame.expand(textBreakStart);
      textBreakStop = UIFrame.expand(textBreakStop);
    }
  ...
  public void text(float num, float x, float y) {
    text(UIFrame.nfs(num, 0, 3), x, y);
  }


  public void text(float num, float x, float y, float z) {
    text(UIFrame.nfs(num, 0, 3), x, y, z);
  }
...
//    textZ = 0;  // this will get set by the caller if non-zero
  ...
  protected void textCharImpl(char ch, float x, float y) { //, float z) {
    ...
    if (glyph != null) {
      ...
      showWarning("No glyph found for the " + ch + " (\\u" + UIFrame.hex(ch, 4) + ") character");
    }
  }
 ...
 */
  ...
  public void pushStyle() {
    if (styleStackDepth == styleStack.length) {
      styleStack = (PStyle[]) UIFrame.expand(styleStack);
    }
...
//    backgroundImpl();
  ...
  protected void colorCalcARGB(int argb, float alpha) {
    if (alpha == colorModeA) {
      ...
      calcAi = (int) (((argb >> 24) & 0xff) * UIFrame.constrain((alpha / colorModeA), 0, 1));
...
//  }
  ...
  static public int lerpColor(int c1, int c2, float amt, int mode) {
    ...
    if (mode == RGB) {
      ...
      return ((UIFrame.round(a1 + (a2-a1)*amt) << 24) |
              (UIFrame.round(r1 + (r2-r1)*amt) << 16) |
              (UIFrame.round(g1 + (g2-g1)*amt) << 8) |
              (UIFrame.round(b1 + (b2-b1)*amt)));
      ...
      float a1 = (c1 >> 24) & 0xff;
      float a2 = (c2 >> 24) & 0xff;
      int alfa = (UIFrame.round(a1 + (a2-a1)*amt)) << 24;
      ...
      float ho = (UIFrame.lerp(lerpColorHSB1[0], lerpColorHSB2[0], amt)) % 1.0f;
      */
      float ho = UIFrame.lerp(lerpColorHSB1[0], lerpColorHSB2[0], amt);
      float so = UIFrame.lerp(lerpColorHSB1[1], lerpColorHSB2[1], amt);
      float bo = UIFrame.lerp(lerpColorHSB1[2], lerpColorHSB2[2], amt);
  ...
  protected static class AsyncImageSaver {
    ...
    public void saveTargetAsync(final PGraphics renderer, final PImage target, // ignore
      ...
      if (target.parent.frameCount - 1 == lastFrameCount && TARGET_COUNT > 1) {
        ...
        long delay = UIFrame.round((lastTime + avgTimePerFrame - now) / 1e6f);
        try {
